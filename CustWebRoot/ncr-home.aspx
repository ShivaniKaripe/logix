...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "ncr-home.aspx"
  ...
  If (Request.Form("password") <> "" And Request.Form("identifier") <> "" And Session("customerpk") <> vbNull) Then
    ...
    If (IsNumeric(Request.Form("identifier"))) Then
      ...
      MyCommon.QueryStr = "SELECT C.CustomerPK,C.PrimaryExtID,C.Employee,C.CardStatusID,CE.Email " & _
                          ...
                          "WHERE PrimaryExtID='" & PrimaryExtID & "' and Password ='" & MyCryptLib.SQL_StringEncrypt(Request.Form("password")) & "'"
    Else
      'The identifier the customer supplied isn't just numbers, so assume an email address
      MyCommon.QueryStr = "SELECT C.CustomerPK,C.PrimaryExtID,C.Employee,C.CardStatusID,CE.Email " & _
                          "FROM Customers AS C " & _
                          "LEFT JOIN CustomerExt AS CE on CE.CustomerPK=C.CustomerPK " & _
                          "WHERE Email='" & Request.Form("identifier") & "' and Password ='" & MyCryptLib.SQL_StringEncrypt(Request.Form("password")) & "'"
    End If
    ...
    If (rst.Rows.Count() > 0) Then
      ...
      Session.Remove("customerpk")
      Session.Add("customerpk", rst.Rows(0).Item("customerpk"))
    End If
  ElseIf (Request.Form("opt-type") <> "") Then
  ElseIf (Request.QueryString("Logout") = "logout") Then
    Session.Abandon()
    Response.Status = "301 Moved Permanently"
    Response.AddHeader("Location", "ncr-index.aspx")
  Else
    Response.Status = "301 Moved Permanently"
    Response.AddHeader("Location", "ncr-index.aspx")
  End If
...
%>
<script type="text/javascript" language="javascript">
  xmlhttpPost('ncr-cwfeed.aspx',"info","sidebar")
  xmlhttpPost('ncr-cwfeed.aspx',"cust","main")
</script>
<%
