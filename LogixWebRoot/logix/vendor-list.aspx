...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "vendor-list.aspx"
  ...
  Try
    ...
    Dim SortText As String = "ExtVendorID"
    ...
    sSearchQuery = "select VendorID, ExtVendorID, Name, LastUpdate " & _
                   "from Vendors CV with (NoLock) "
    ...
    If (idSearchText <> "") Then
      ...
      sSearchQuery = sSearchQuery & " where AnyVendor=0 and Deleted=0 and (ExtVendorId = '" & idSearchText & "'"
      sSearchQuery = sSearchQuery & " or Name like N'%" & idSearchText & "%'"
      sSearchQuery = sSearchQuery & " or VendorID like N'%" & idSearchText & "%')"
    Else
      sSearchQuery &= " where AnyVendor=0 and Deleted=0 "
    End If
    ...
    If (sizeOfData = 1 AndAlso Request.QueryString("searchterms") <> "") Then
      ...
      Response.AddHeader("Location", "vendor-edit.aspx?VendorID=" & dst.Rows(i).Item("VendorID"))
    End If
%>
<div id="intro">
  <h1 id="title">
    <% Sendb(Copient.PhraseLib.Lookup("term.vendors", LanguageID))%>
  </h1>
  <div id="controls">
    <form action="vendor-edit.aspx" id="controlsform" name="controlsform">
    <%
...
<div id="main">
  ...
  <table class="list" summary="<% Sendb(Copient.PhraseLib.Lookup("term.vendors", LanguageID)) %>">
    <thead>
      <tr>
        <th align="left" class="th-id" scope="col">
          <a href="vendor-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=VendorID&amp;SortDirection=<% Sendb(SortDirection) %>">
            <% Sendb(Copient.PhraseLib.Lookup("term.id", LanguageID))%>
          </a>
          <%
            If SortText = "VendorID" Then
        ...
        <th align="left" class="th-code" scope="col">
          <a href="vendor-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=ExtVendorID&amp;SortDirection=<% Sendb(SortDirection) %>">
            <% Sendb(Copient.PhraseLib.Lookup("term.code", LanguageID))%>
          </a>
          <%
            If SortText = "ExtVendorID" Then
              If SortDirection = "ASC" Then
                Sendb("<span class=""sortarrow"">&#9660;</span>")
              Else
                Sendb("<span class=""sortarrow"">&#9650;</span>")
        ...
        <th align="left" class="th-name" scope="col">
          <a href="vendor-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=Name&amp;SortDirection=<% Sendb(SortDirection) %>">
        ...
        <th align="left" class="th-datetime" scope="col">
          <a href="vendor-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=LastUpdate&amp;SortDirection=<% Sendb(SortDirection) %>">
    ...
    <tbody>
      <%
        ...
        While (i < sizeOfData And i < linesPerPage + linesPerPage * PageNum)
          ...
          Send("        <td>" & dst.Rows(i).Item("VendorID") & "</td>")
          Send("        <td>" & MyCommon.SplitNonSpacedString(MyCommon.NZ(dst.Rows(i).Item("ExtVendorID"), "&nbsp;"), 25) & "</td>")
          Send("        <td><a href=""vendor-edit.aspx?VendorID=" & dst.Rows(i).Item("VendorID") & """>" & MyCommon.SplitNonSpacedString(dst.Rows(i).Item("Name"), 30) & "</a></td>")
