...
<%
  ' *****************************************************************************
  ' * FILENAME: offer-rew-tender-point.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Dim sTenderPointId As String
  Dim sTenderPointIds As String
  Dim bOnlyOneTenderPointProgram As Boolean = True
  ...
  Response.Expires = 0
  MyCommon.AppName = "offer-rew-tender-point.aspx"
  ...
  If sTenderPointIds = "" Then
    sTenderPointIds = "0"
  End If
  MyCommon.QueryStr = "select ProgramID from PointsPrograms with (NoLock) where deleted=0 and ProgramID in (" & sTenderPointIds & ") order by ProgramName;"
  rst2 = MyCommon.LRT_Select
  If rst2.Rows.Count > 0 Then
    sTenderPointId = rst2.Rows(0).Item("ProgramID")
    If rst2.Rows.Count > 1 Then
      bOnlyOneTenderPointProgram = False
    End If
  Else
    sTenderPointId = "0"
    infoMessage = Copient.PhraseLib.Lookup("offer-rew-tender-point.NoProgramDefined", LanguageID)
  End If
  ...
  Send_HeadBegin("term.offer", "term.tenderpointsreward", OfferID)
...
{
    //handleKeyUp(99999);
    xmlhttpPost('OfferFeeds.aspx','ProductGroupsCM');
...
<%
    MyCommon.QueryStr = "select ProgramID,ProgramName from PointsPrograms with (NoLock) where deleted=0 and ProgramID in (" & sTenderPointIds & ") order by ProgramName;"
    rst2 = MyCommon.LRT_Select
...
function PupdateButtons(){
  ...
  var Pselect1Obj = document.getElementById('Pselect1');
  var Pdeselect1Obj = document.getElementById('Pdeselect1');
  
  if (elemDisallowEditPpOpt != null && elemDisallowEditPpOpt.value == '1') {
      Pselect1Obj.disabled=true;
      Pdeselect1Obj.disabled=true;
  } else {
    if (Pselect1Obj != null && Pdeselect1Obj != null) {
      if (selectObj.length == 0) {
        Pselect1Obj.disabled=false;
        Pdeselect1Obj.disabled=true;
      } else {
        Pselect1Obj.disabled=false;
        Pdeselect1Obj.disabled=false;
      }
    }
  }
}
...
function submitenter(e) {
	...
	// if key is not numeric, then return false
	// 48 = '0' and 57 = '9'
...
<form action="offer-rew-tender-point.aspx" id="mainform" name="mainform" onkeydown="if (event.keyCode == 13) return false;" onsubmit="return saveForm();">
  <div id="intro">
    ...
    <input type="hidden" id="IsTemplate" name="IsTemplate" value="<% 
        ...
        %>" />
    <%If (IsTemplate) Then
        Send("<h1 id=""title"">" & Copient.PhraseLib.Lookup("term.template", LanguageID) & " #" & OfferID & " " & StrConv(Copient.PhraseLib.Lookup("term.tenderpointsreward", LanguageID), VbStrConv.Lowercase) & "</h1>")
      Else
        Send("<h1 id=""title"">" & Copient.PhraseLib.Lookup("term.offer", LanguageID) & " #" & OfferID & " " & StrConv(Copient.PhraseLib.Lookup("term.tenderpointsreward", LanguageID), VbStrConv.Lowercase) & "</h1>")
      End If
  ...
  <div id="main">
    ...
    <div id="column1">
      <div class="box" id="selector">
        ...
        <input type="radio" id="functionradio1" name="functionradio" <%Sendb(IIf(MyCommon.Fetch_SystemOption(175)="1", " checked=""checked"" ", "")) %>
          <% sendb(disabledattribute) %> /><label for="functionradio1"><% Sendb(Copient.PhraseLib.Lookup("term.startingwith", LanguageID))%></label>

        <input type="radio" id="functionradio2" name="functionradio" <%Sendb(IIf(MyCommon.Fetch_SystemOption(175)="2", " checked=""checked"" ", "")) %>
      ...
      <%
              ...
              MyCommon.QueryStr = "select " & topString & " ProductGroupID,CreatedDate,Name,PhraseID,LastUpdate,AnyProduct from ProductGroups with (NoLock) where Deleted=0" & Limiter' & " order by AnyProduct desc, Name"
              ...
              For Each row In rst.Rows
                  If MyCommon.NZ(row.Item("ProductGroupID"), 0) = 1 AndAlso (ExcludedItem <> 1 AndAlso SelectedItem <> 1) Then
                      Send("<option value=""" & row.Item("ProductGroupID") & """ style=""color: brown; font-weight: bold;"">" & row.Item("Name") & "</option>")
                  Else
                  Sendb("<option value=""" & row.Item("ProductGroupID") & """ title=""" & row.Item("Name") & """>" & row.Item("Name") & "</option>")
          End If
              Next
      %>
          </select>
        ...
        <input class="regular deselect" id="deselect1" name="deselect1" type="button" value="<% Sendb(Copient.PhraseLib.Lookup("term.deselect", LanguageID))%> &#9650;" disabled="disabled" onclick="handleSelectClick('deselect1');" /><br />
        <br class="half" />
        ...
        <input class="regular select" name="select2" id="select2" type="button" value="&#9660; <% Sendb(Copient.PhraseLib.Lookup("term.select", LanguageID))%>" disabled="disabled" onclick="handleSelectClick('select2');"<% sendb(disabledattribute) %> />&nbsp;
        <input class="regular deselect" name="deselect2" id="deselect2" type="button" value="<% Sendb(Copient.PhraseLib.Lookup("term.deselect", LanguageID))%> &#9650;" disabled="disabled" onclick="handleSelectClick('deselect2');"<% sendb(disabledattribute) %> /><br />
        <br class="half" />
        <select class="longer" id="excluded" name="excluded" size="2"<% sendb(disabledattribute) %>>
          <%
            ...
            rst2 = MyCommon.LRT_Select
            For Each row2 In rst2.Rows
              Send("<option value=""" & row2.Item("ExcludedProdGroupID") & """>" & row2.Item("Name") & "</option>")
              ExcludedItem = row2.Item("ExcludedProdGroupID")
            Next
          %>
        </select>
        <hr class="hidden" />
    ...
    <div id="column2">
      <div class="box" id="programs">
        <h2>
          ...
          <% If Not bOnlyOneTenderPointProgram Then%>
            <% If (IsTemplate) Then%>
            <span class="tempRequire">
        ...
        <%
          ...
          rst2 = MyCommon.LRT_Select
          If bOnlyOneTenderPointProgram Then
            If rst2.Rows.Count = 0 Then
              MyCommon.QueryStr = "update RewardPoints with (RowLock) set ProgramID=" & sTenderPointId & " where RewardPointsID=" & LinkID
              MyCommon.LRT_Execute()
              MyCommon.QueryStr = "select PP.ProgramID,PP.ProgramName from PointsPrograms as PP with (NoLock) " & _
                                  "inner join RewardPoints as RP with (NoLock) on RP.ProgramID=PP.ProgramID " & _
                                  "inner join OfferRewards as OFR with (NoLock) on OFR.LinkID=RP.RewardPointsID " & _
        ...
        <%
          ...
          For Each row In rst.Rows
            ...
            If (row.Item("UseSpecialPricing") <> True) Then
              ...
              Else
                Sendb("<label for=""tier" & q & """>" & Copient.PhraseLib.Lookup("term.tier", LanguageID) & q & ":</label> <input class=""shorter"" id=""tier" & q & """ name=""tier" & q & """ type=""text"" onkeypress=""return submitenter(event)"" style=""width:35px;"" value=""" & Int(MyCommon.NZ(row.Item("RewardAmount"), 0)) & """" & sDisabled & " /><br />")
              End If
            End If
            q = q + 1
          Next
          ...
          If rst.Rows.Count > 0 Then
            ...
            For Each row In rst.Rows
              If (row.Item("UseSpecialPricing") = True) Then
                ...
                End If
                Send("    <td><input class=""shorter"" id=""tier" & q & """ name=""tier" & q & """ type=""text"" onkeypress=""return submitenter(event)"" value=""" & Int(MyCommon.NZ(row.Item("RewardAmount"), 0)) & """" & sDisabled & " />")
