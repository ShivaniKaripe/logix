...
<%
  ' *****************************************************************************
  ' * FILENAME: offer-con-preference.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
<script runat="server">
  ...
  Sub Populate_Offer_Data()
    ...
    Common.QueryStr = "select O.Name, O.IsTemplate, O.FromTemplate, O.EngineID from Offers as O with (NoLock) " & _
                      "where O.Deleted=0 and O.OfferID=" & PrefCon.Offer.ID
  ...
  Sub Load_Saved_Prefence()
    ...
    If PrefCon.ConditionID > 0 Then
      ' find the selected preference for this condition
      Common.QueryStr = "select LinkID as PreferenceID, Disallow_Edit from OfferConditions with (NoLock) " & _
                        "where ConditionID=" & PrefCon.ConditionID
      dt = Common.LRT_Select
      If dt.Rows.Count > 0 Then
        PrefCon.SelectedPref.ID = Common.NZ(dt.Rows(0).Item("PreferenceID"), 0)
      If Not (PrefCon.Offer.IsTemplate) Then 
        PrefCon.Offer.DisallowEdit = (Logix.UserRoles.EditOffer = False) _
                                     OrElse (PrefCon.Offer.FromTemplate AndAlso Common.NZ(dt.Rows(0).Item("DisallowEdit"), False))
      Else
        PrefCon.Offer.DisallowEdit = (Logix.UserRoles.EditTemplates = False) 
      End If
  ...
  Sub Send_Page()
    ...
    Common.AppName = "offer-con-preference.aspx"
    ...
    If Common.IsIntegrationInstalled(Copient.CommonInc.Integrations.PREFERENCE_MANAGER) Then
      ...
      Integer.TryParse(GetCgiValue("ConditionID"), PrefCon.ConditionID)
  ...
  Sub Send_Head()
    ...
    Send_Metas()
    Send_Links(Handheld)
    Send_Scripts(New String() {"ajaxSubmit.js", "prefentry.js", "cmprefentry.js"})
    Send("<script type=""text/javascript"">")
        Send("function ChangeParentDocument() { ")
        Send("  if (opener != null) {")
        Send("    var newlocation = 'offer-con.aspx?OfferID=" & PrefCon.Offer.ID & "'; ")
        Send("  if (opener.location.href.indexOf(newlocation) >-1) {")
        Send("  opener.location = 'offer-con.aspx?OfferID=" & PrefCon.Offer.ID & "'; ")
        Send("  }")
        Send("  }")
        Send("} ")
    Send("</" & "script>")
  ...
  Sub Send_Intro()
    ...
    Send("  <input type=""hidden"" id=""ConditionID"" name=""ConditionID"" value=""" & PrefCon.ConditionID & """ />")
  ...
  Sub Send_Controls()
    ...
    If Not (PrefCon.Offer.IsTemplate) Then
       If (Logix.UserRoles.EditOffer And Not (PrefCon.Offer.FromTemplate And PrefCon.Offer.DisallowEdit)) Then
         Send_Save(" onclick=""saveForm();""", "button")
       End If
    Else 
  ...
  Sub Send_Value_Box()
    Send("<div class=""box"" id=""prefvalues""" & IIf(PrefCon.ConditionID > 0 AndAlso PrefCon.SelectedPref.ID > 0, "", " style=""display:none;""") & ">")
    Send("  <h2>")
  ...
  Sub Send_Value_Load_JS()
    ...
    Send("    var formdata = 'mode=loadCMPrefConValues&offerid=" & PrefCon.Offer.ID & "&conditionid=" & PrefCon.ConditionID & "&prefid=' + getSelectedValue(0) + '&edit=" & IIf(PrefCon.Offer.DisallowEdit, "-1", "1") & "';")
    Send("")
    ...
    Send("    storeAllOperations(1);")
    Send("    refreshDateValueBox(1);")
    Send("  }")
    Send("")
    ...
    Send("      xmlhttpPost('preference-feeds.aspx?mode=saveCMPrefCon', 'mainform', 'infobar', '" & Copient.PhraseLib.Lookup("message.Saving", LanguageID) & "');")
    Send("    }")
    Send("  }")
    Send("")
    Send("  function validateForm() {")
    Send("    var tierlevels = 1;")
  ...
  Sub Send_Closing_JS()
    ...
    Send("  removeUsed(true);")
    Send("  updateButtons();")
    If PrefCon.ConditionID > 0 AndAlso PrefCon.SelectedPref.ID > 0 Then
      Send("  loadValues();")
    End If
    Send("</" & "script>")
  End Sub
