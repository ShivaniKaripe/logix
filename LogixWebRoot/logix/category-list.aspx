...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "category-list.aspx"
  ...
  Try
    ...
    Dim SortText As String = "Description"
    ...
    sSearchQuery = "select OfferCategoryID, Description, ExtCategoryID, BaseOfferID, LastUpdate " & _
                   "from OfferCategories as OC with (NoLock) " & _
                   "where Deleted=0 and OfferCategoryID>=0"
    ...
    If (idSearchText <> "") Then
      ...
      sSearchQuery = sSearchQuery & " and (ExtCategoryID='" & idSearchText & "'"
      sSearchQuery = sSearchQuery & " or Description like N'%" & idSearchText & "%'"
      sSearchQuery = sSearchQuery & " or OfferCategoryID like N'%" & idSearchText & "%'"
      sSearchQuery = sSearchQuery & " or BaseOfferID like N'%" & idSearchText & "%')"
    End If
    ...
    If (sizeOfData = 1 AndAlso Request.QueryString("searchterms") <> "") Then
      ...
      Response.AddHeader("Location", "category-edit.aspx?OfferCategoryID=" & dst.Rows(i).Item("OfferCategoryID"))
    End If
%>
<div id="intro">
  <h1 id="title">
  ...
  <div id="controls">
    <form action="category-edit.aspx" id="controlsform" name="controlsform">
    <%
      If (Logix.UserRoles.EditCategories = True) Then
        Send_New()
      End If
    %>
    </form>
...
<div id="main">
  ...
  %>
  <table class="list" summary="<% Sendb(Copient.PhraseLib.Lookup("term.categories", LanguageID)) %>">
    <thead>
      <tr>
        <th align="left" class="th-id" scope="col">
          <a href="category-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=OfferCategoryID&amp;SortDirection=<% Sendb(SortDirection) %>">
            <% Sendb(Copient.PhraseLib.Lookup("term.id", LanguageID))%>
          </a>
          <%
            If SortText = "OfferCategoryID" Then
        ...
        <th align="left" class="th-code" scope="col">
          <a href="category-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=ExtCategoryID&amp;SortDirection=<% Sendb(SortDirection) %>">
            <% Sendb(Copient.PhraseLib.Lookup("term.code", LanguageID))%>
          </a>
          <%
            If SortText = "ExtCategoryID" Then
              If SortDirection = "ASC" Then
                Sendb("<span class=""sortarrow"">&#9660;</span>")
              Else
                Sendb("<span class=""sortarrow"">&#9650;</span>")
        ...
        <th align="left" class="th-name" scope="col">
          <a href="category-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=Description&amp;SortDirection=<% Sendb(SortDirection) %>">
        ...
        <th align="left" class="th-name" scope="col">
          <a href="category-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=BaseOfferID&amp;SortDirection=<% Sendb(SortDirection) %>">
            <% Sendb(Copient.PhraseLib.Lookup("term.baseoffer", LanguageID))%>
          </a>
          <%
            If SortText = "BaseOfferID" Then
              If SortDirection = "ASC" Then
                Sendb("<span class=""sortarrow"">&#9660;</span>")
              Else
                Sendb("<span class=""sortarrow"">&#9650;</span>")
        ...
        <th align="left" class="th-datetime" scope="col">
          <a href="category-list.aspx?searchterms=<%sendb(Request.QueryString("searchterms")) %>&amp;SortText=LastUpdate&amp;SortDirection=<% Sendb(SortDirection) %>">
    ...
    <tbody>
      <%
        ...
        While (i < sizeOfData And i < linesPerPage + linesPerPage * PageNum)
          ...
          Send("        <td>" & dst.Rows(i).Item("OfferCategoryID") & "</td>")
          Send("        <td>" & MyCommon.SplitNonSpacedString(MyCommon.NZ(dst.Rows(i).Item("ExtCategoryID"), "&nbsp;"), 25) & "</td>")
          Send("        <td><a href=""category-edit.aspx?OfferCategoryID=" & dst.Rows(i).Item("OfferCategoryID") & """>" & MyCommon.SplitNonSpacedString(dst.Rows(i).Item("Description"), 30) & "</a></td>")
          If (MyCommon.NZ(dst.Rows(i).Item("BaseOfferID"), 0) > 0) Then
            Send("        <td><a href=""offer-redirect.aspx?OfferID=" & MyCommon.NZ(dst.Rows(i).Item("BaseOfferID"), 0) & """>" & MyCommon.NZ(dst.Rows(i).Item("BaseOfferID"), 0) & "</a></td>")
          Else
