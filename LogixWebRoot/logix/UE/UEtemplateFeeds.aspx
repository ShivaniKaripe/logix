...
<script runat="server">
  ...
  Sub GenerateFieldList(ByVal OfferID As String, ByVal DeliverableID As String, ByVal PageName As String, ByVal PageEditable As Boolean)
    ...
    Try
            MyCommon.QueryStr = "select TFP.DeliverableID, UI.FieldID, UI.FieldName, TFP.Editable from UIFields UI with (NoLock) " & _
                          "left join TemplateFieldPermissions TFP with (NoLock) on TFP.FieldID = UI.FieldID and TFP.OfferID = " & OfferID & " " & _
                          "where UI.PageName = '" & PageName & "';"
      ...
      If (rst.Rows.Count > 0) Then
        ...
        For Each row In rst.Rows
                    ...
                    If DeliverableID IsNot Nothing AndAlso DeliverableID = 0 Then
                        CheckedAttr = ""
...
<%
  ...
  MyCommon.AppName = "UEtemplateFeeds.aspx"
  
  If (Request.QueryString("PageName") <> "") Then
    Response.Expires = 0
    Response.Clear()
    Response.ContentType = "text/html"
    GenerateFieldList(Request.QueryString("OfferID"), Request.QueryString("DeliverableID"), Request.QueryString("PageName"), Boolean.Parse(Request.QueryString("PageEditable")))
  Else
