...
<%
  ...
  Dim dtFailoverHist As System.Data.DataTable = Nothing
  ...
  Dim IsUEInstalled As Boolean = False
  ...
  Dim maxRectoShow As Integer = 0
    Dim operateAtEnterprise As Boolean = (MyCommon.Fetch_UE_SystemOption(91) = "1")
    Dim UseHealthService As Boolean = (MyCommon.Fetch_UE_SystemOption(188) = "1")
    
  If Request IsNot Nothing AndAlso Request.Browser IsNot Nothing AndAlso Request.Browser.Platform IsNot Nothing AndAlso Request.ServerVariables IsNot Nothing AndAlso Request.ServerVariables("HTTP_USER_AGENT") IsNot Nothing Then
    Handheld = DetectHandheld(Request.Browser("IsMobileDevice"), Request.Browser.Platform.ToString(), Request.ServerVariables("HTTP_USER_AGENT").ToString())
  End If

    If operateAtEnterprise AndAlso UseHealthService Then
        Response.Redirect("UEServerHealthSummary.aspx")
    End If
    
    Response.Expires = 0
    MyCommon.AppName = "store-health-ue.aspx"
    ...
    IsUEInstalled = MyCommon.IsEngineInstalled(Copient.CommonInc.InstalledEngines.UE)

    If (Request.QueryString("action") <> "" AndAlso Request.QueryString("locid") <> "") Then
        If (Request.QueryString("action") = "mute") Then
            MyCommon.QueryStr = "update Locations with (RowLock) Set SendAlert = 0 where LocationID = " & Server.HtmlEncode(Request.QueryString("locid"))
            MyCommon.LRT_Execute()
        ElseIf (Request.QueryString("action") = "unmute") Then
            MyCommon.QueryStr = "update Locations with (RowLock) Set SendAlert = 1 where LocationID = " & Server.HtmlEncode(Request.QueryString("locid"))
            MyCommon.LRT_Execute()
        ElseIf (Request.QueryString("action") = "disable") Then
            MyCommon.QueryStr = "update Locations with (RowLock) Set HealthReported = 0 where LocationID = " & Server.HtmlEncode(Request.QueryString("locid"))
            MyCommon.LRT_Execute()
        ElseIf (Request.QueryString("action") = "enable") Then
            MyCommon.QueryStr = "update Locations with (RowLock) Set HealthReported = 1 where LocationID = " & Server.HtmlEncode(Request.QueryString("locid"))
            MyCommon.LRT_Execute()
        End If
    End If
    ...
    If (Request.QueryString("filterhealth") <> "") Then
        FilterHealth = MyCommon.Extract_Val(Server.HtmlEncode(Request.QueryString("filterhealth")))
        Select Case FilterHealth
            Case 1 ' communications ok
                SearchClause &= " and DBOK = 1 and isnull(" & CommsFilter & ",1) = 1 "
            ...
            Case 3 ' central exceptions only
                SearchClause &= "  and (DBOK = 0 or isnull(" & CommsFilter & ",0) = 0) and HealthReported = 1 and ls.LocalServerID is not null "
    ...
    'Active failover
    If (FilterHealth = 5) Then
        MyCommon.QueryStr = "select ls.LocationID, ls.LocalServerID, '' as LocationName, loc.EngineID, loc.ExtLocationCode,loc.HealthReported, PE.Description as EngineName, ls.FailoverServer,ls.MustIPL, " & _
                            "IsNull(loc.SendAlert,0) SendAlert,0 as Sev1Errors, 0 as Sev10Errors,1 as Severity,1 as SeveritySort,0 as Comms " & _
                            "from LocalServers as ls with (NoLock) " & _
                            "inner join Locations as loc with (NoLock) on loc.LocationID = ls.LocationID " & _
                            "inner join PromoEngines PE with (NoLock) on PE.EngineID=loc.EngineID " & _
                            "where FailoverServer = 1 and loc.EngineID = 9 and loc.Deleted=0" & IIf(operateAtEnterprise, " and loc.locationtypeid = 2 ", "")
        'Failover History
    ElseIf (FilterHealth = 6) Then
        MyCommon.QueryStr = "select distinct loc.LocationID, 0 as LocalServerID,  loc.LocationName, loc.EngineID, loc.ExtLocationCode,loc.HealthReported, PE.Description as EngineName,0 as FailoverServer,Null as MustIPL, " & _
                             "IsNull(loc.SendAlert,0) SendAlert,0 as Sev1Errors, 0 as Sev10Errors,1 as Severity,1 as SeveritySort,0 as Comms  from Locations as loc with (nolock) " & _
                             "inner join PromoEngines PE with (NoLock) on PE.EngineID=loc.EngineID " & _
                             "inner join FailOverHistory F on F.LocationID = loc.LocationID  " & _
                             "where loc.EngineID = 9 and loc.Deleted=0 " & IIf(operateAtEnterprise, " and loc.locationtypeid=2 ", "")
    Else
        MyCommon.QueryStr = "select loc.LocationID, ls.LocalServerID, loc.LocationName, loc.EngineID, loc.ExtLocationCode, loc.HealthReported, PE.Description as EngineName, ls.CMLastHeard, ls.LastHeard, ls.FailoverServer, ls.MustIPL, ls.SanityCheckLastHeard, " & _
                            ...
                            "case " & _
                            "  when ls.LocalServerID > -1 and (ls.Sev1Errors > 0 or DATEADD(n, " & CentralHighValue & ", LS.LastHeard) <= getDate() or DATEADD(n," & CentralHighValue & ", LS.IncentiveLastHeard) <=getDate() or DATEADD(n," & CentralHighValue & ",LS.TransactionLastHeard) <=getDate() or DATEADD(n," & CentralHighValue & ", LS.TransDownLoadLastHeard) <=getDate()) then 1 " & _
                            ...
                            "  when ls.LocalServerID > -1 and (ls.Sev10Errors > 0 or DATEADD(n," & CentralLowValue & ", LS.LastHeard) <= getDate() or DATEADD(n," & CentralLowValue & ", LS.IncentiveLastHeard) <=getDate() or DATEADD(n," & CentralLowValue & ",LS.TransactionLastHeard) <=getDate()  or DATEADD(n," & CentralLowValue & ", LS.TransDownLoadLastHeard) <=getDate() or (DBOK=0)) then 10 " & _
                            "  else 0 " & _
                            "end as Severity, " & _
                            "case " & _
                            "  when ls.LocalServerID > -1 and (ls.Sev1Errors > 0 or DATEADD(n, " & CentralHighValue & ", LS.LastHeard) <= getDate() or DATEADD(n," & CentralHighValue & ", LS.IncentiveLastHeard) <=getDate() or DATEADD(n," & CentralHighValue & ",LS.TransactionLastHeard) <=getDate() or DATEADD(n," & CentralHighValue & ", LS.TransDownLoadLastHeard) <=getDate()) then 1 " & _
                            ...
                            "where loc.EngineID = 9 and loc.Deleted=0 " & SearchClause & " " & IIf(operateAtEnterprise, " and loc.locationtypeid=2 ", "")
                     
    End If

    If (FilterHealth <> 5 AndAlso FilterHealth <> 6) Then
    ...
    Send("<!-- QueryStr=" & MyCommon.QueryStr & " -->")
...
<script type="text/javascript" language="javascript">
  function loadStoreHealth() {
    ...
    var engine = 9;
    var pageUrl = 'store-health-ue.aspx';
    ...
    switch (engine) {
      ...
      case 2:
        ...
        break;
      case 9:
        pageUrl = 'store-health-ue.aspx?filterhealth=2';
        break;
      default:
        pageUrl = 'store-health-ue.aspx?filterhealth=2';
        break;
...
<div id="intro">
  <%
    ...
    ElseIf (dst.Rows.Count > 1) Then
      ...
      For Each row In dst.Rows
        Send("  <option value=""" & MyCommon.NZ(row.Item("EngineID"), -1) & """ " & IIf(MyCommon.NZ(row.Item("EngineID"), -1) = 9, "selected", "") & ">" & Copient.PhraseLib.Lookup(MyCommon.NZ(row.Item("PhraseID"), 0), LanguageID) & "</option>")
      Next
      Send("</select>")
    End If
  %>
  <div id="controls" style="width: 0px;">
    <% If (Logix.UserRoles.AccessLogs) Then%>
    <form action="<%Sendb("/logix/log-view.aspx?filetype=-1&amp;fileyear=" & Year(Today) & "&amp;filemonth=" & Month(Today) & "&amp;fileday=" & Day(Today)) %>"
    id="controlsform" name="controlsform" target="_blank" style="float: right;">
    ...
    <thead>
      <tr>
        ...
        <th align="left" class="th-code" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=loc.ExtLocationCode&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="left" class="th-id" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=loc.LocationID&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="left" class="th-id" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=ls.LocalServerID&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="left" class="th-severity" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=SeveritySort&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="left" class="th-errors" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=Sev1Errors&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th class="th-failover" scope="col" style="text-align: center; display: none;" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=ls.FailoverServer&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th class="th-failover" scope="col" style="text-align: center; display: none;" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=Comms&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="center" class="th-id" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=HealthReported&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        <th align="center" class="th-alert" scope="col" valign="bottom">
          <a href="store-health-ue.aspx?searchterms=<%sendb(Server.HtmlEncode(Request.QueryString("searchterms"))) %>&action=Sort&SortText=SendAlert&SortDirection=<% Sendb(SortDirection) %>&filterhealth=<% Sendb(FilterHealth) %>">
        ...
        If (dstStoreList.Rows.Count > 0) Then
          While (i < sizeOfData And i < linesPerPage + linesPerPage * PageNum)
            ...
            If (MyCommon.NZ(row.Item("Severity"), -1) > 0) Then
              ...
              Send("<a href=""javascript:toggleRow(" & i & ");"">")
              Send("  <img id=""img" & i & """ src=""../../images/plus2.png"" border=""0"" alt=""" & Copient.PhraseLib.Lookup("store-health-ue.ViewHideErrorDetails", LanguageID) & """ title=""" & Copient.PhraseLib.Lookup("store-health-ue.ViewHideErrorDetails", LanguageID) & """ />")
              Send("</a>")
              Send("</td>")
            Else
              Send("<td align=""center"">")
            ...
            If FilterHealth = 5 Then
              MyCommon.QueryStr = "  select 'term.local' as ServerType, 1 as ServerTypeID, 1 as HealthSeverityID, 'LS455' as ErrorCode, 455 as ErrorID, DATEDIFF(mi,failoverstart, getdate()) as MinutesInError, " & _
                                 " 'In failover mode, Server #='+ CONVERT(varchar(50),[LocalServerID]) + ', IP=' + [IP] as ErrorText,[FailOverSubject] as TimeStamp,'' as TagName, '' as SectionName   from failoverhistory where FailOverEnd is null and locationid= " & LID & "  and localserverid= " & LocalServerID
              dtFailoverHist = MyCommon.LRT_Select
            ElseIf FilterHealth = 6 Then
              maxRectoShow = MyCommon.Extract_Val(MyCommon.Fetch_SystemOption(155))
              'max records in db is configured in system option table; if not configured then default 10 records need to be display
              MyCommon.QueryStr = "  select Top " & IIf(maxRectoShow <= 0, 10, maxRectoShow) & "'term.local' as ServerType, 1 as ServerTypeID, 1 as HealthSeverityID, 'LS455' as ErrorCode, 455 as ErrorID, DATEDIFF(mi,failoverstart, isnull(failoverend,getdate())) as MinutesInError, " & _
                               " 'Failover, Server #='+ CONVERT(varchar(50),[LocalServerID]) + ', IP=' + [IP] as ErrorText,[FailOverSubject] as TimeStamp,'' as TagName, '' as SectionName,FailOverHistoryID   from failoverhistory where locationid= " & LID & " order by failoverhistoryid desc"
              dtFailoverHist = MyCommon.LRT_Select
            ElseIf FilterHealth = 0 Then
              MyCommon.QueryStr = "  select 'term.local' as ServerType, 1 as ServerTypeID, 1 as HealthSeverityID, 'LS455' as ErrorCode, 455 as ErrorID, DATEDIFF(mi,failoverstart, getdate()) as MinutesInError, " & _
                                  " 'Last failover, Server #='+ CONVERT(varchar(50),[LocalServerID]) + ', IP=' + [IP] + ', <br> LS Timestamp=' +  [FailOverSubject] as ErrorText,'' as TagName, '' as SectionName   from failoverhistory where FailOverEnd is null and locationid= " & LID & " and localserverid= " & LocalServerID
              dtFailoverHist = MyCommon.LRT_Select
            End If

            ' Severity And Error text
            If (LocalServerID > -1 AndAlso MyCommon.NZ(row.Item("Severity"), -1) > 0) Then
              ...
              SeverityDesc = ""
              'local server errors
              If FilterHealth = 6 Or FilterHealth = 5 Then
                Sev1Errs = dtFailoverHist.Rows.Count
              ElseIf FilterHealth = 0 Then
                Sev1Errs = MyCommon.NZ(row.Item("Sev1Errors"), 0) + dtFailoverHist.Rows.Count
              Else
            ...
            End If
            ' Hide columns for Active Failovers and Failover History
            If (FilterHealth <> 5 AndAlso FilterHealth <> 6) Then
              ' Report Health column
              If (row.Item("HealthReported") = True) Then
                Send("     <td align=""center""><a href=""?pagenum=" & PageNum & "&action=disable&locid=" & row.Item("LocationID") & "&filterhealth=" & FilterHealth & """><img src=""../../images/report-on.png"" border=""0"" alt=""" & Copient.PhraseLib.Lookup("store-health-ue.DisableReporting", LanguageID) & """ title=""" & Copient.PhraseLib.Lookup("store-health-ue.DisableReporting", LanguageID) & """ /></a></td>")
              Else
                Send("     <td align=""center""><a href=""?pagenum=" & PageNum & "&action=enable&locid=" & row.Item("LocationID") & "&filterhealth=" & FilterHealth & """><img src=""../../images/report-off.png"" border=""0"" alt=""" & Copient.PhraseLib.Lookup("store-health-ue.EnableReporting", LanguageID) & """ title=""" & Copient.PhraseLib.Lookup("store-health-ue.EnableReporting", LanguageID) & """ /></a></td>")
              End If

              ' Send Alert column
              If (row.Item("SendAlert") = True) Then
                Send("     <td align=""center""><a href=""?pagenum=" & PageNum & "&action=mute&locid=" & row.Item("LocationID") & """><img src=""../../images/unmute.png"" border=""0"" alt=""" & Copient.PhraseLib.Lookup("health.mute", LanguageID) & """ title=""" & Copient.PhraseLib.Lookup("health.mute", LanguageID) & """ /></a></td>")
              Else
                Send("     <td align=""center""><a href=""?pagenum=" & PageNum & "&action=unmute&locid=" & row.Item("LocationID") & """><img src=""../../images/mute.png"" border=""0"" alt=""" & Copient.PhraseLib.Lookup("health.unmute", LanguageID) & """ title=""" & Copient.PhraseLib.Lookup("health.unmute", LanguageID) & """ /></a></td>")
              End If
            End If
            ...
            If (LocalServerID > -1 AndAlso MyCommon.NZ(row.Item("Severity"), -1) > 0) Then
              ...
              Send("      <th>" & Copient.PhraseLib.Lookup("term.errcode", LanguageID) & "</th>")
              Send("      <th>" & Copient.PhraseLib.Lookup("term.description", LanguageID) & "</th>")
              'Local Server time only need to show for active failover and Failover history
              If FilterHealth = 6 Or FilterHealth = 5 Then
                Send("      <th>" & Copient.PhraseLib.Lookup("term.timestamp", LanguageID) & "</th>")
              End If
              Send("      <th style=""width:180px;"">" & Copient.PhraseLib.Lookup("term.duration", LanguageID) & "</th>")
              Send("    </tr>")

              dst2 = Nothing
              If FilterHealth = 5 Or FilterHealth = 6 Then
                dst2 = dtFailoverHist
              Else
                ...
                If FilterHealth = 0 Then
                  dst2.Merge(dtFailoverHist)
                End If
              End If
              'sorting is different for failover history
              If (FilterHealth = 6) Then
                rows = dst2.Select("", "failoverhistoryid asc")
              Else
              ...
              For Each row2 In rows
                ...
                If FilterHealth = 6 Or FilterHealth = 5 Then
                  Send("      <td>" & MyCommon.NZ(row2.Item("TimeStamp"), "&nbsp;") & "</td>")
                End If
                Send("      <td>" & IIf(MinutesInError = 0, "0  " & Copient.PhraseLib.Lookup("term.minutes", LanguageID), GetDurationText(MinutesInError)) & "</td>")
                Send("    </tr>")
