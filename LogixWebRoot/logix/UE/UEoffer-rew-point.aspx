...
<%
    ' *****************************************************************************
    ' * FILENAME: UEoffer-rew-point.aspx 
    ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	Dim FractionalQuantity As Decimal = 0
    Dim QuantityDisplay As String = ""
	Dim TierQuantity As Decimal = 0
    ...
    Response.Expires = 0
    MyCommon.AppName = "UEoffer-rew-point.aspx"
	...
	If (Decimal.TryParse(QuantityDisplay, FractionalQuantity)) Then
    FractionalQuantity = Decimal.Parse(QuantityDisplay)
    ...
    If TierLevels > 1 And SupportGlobalAndTieredConditions = 1 Then
        MyCommon.QueryStr = "select TierLevel, Quantity from CPE_DeliverablePointTiers where DPPKID=" & DPPKID & " order by TierLevel;"
        rst = MyCommon.LRT_Select
        If rst.Rows.Count > 0 Then
        ...
        If ValidTiers = True Then
            If (OfferID > 0 AndAlso RewardID > 0 AndAlso ProgramID > 0) Then
                ...
                If MyCommon.Extract_Val(Request.QueryString("DeliverableID")) = 0 Then
                    ' Create a new reward
					If (Create_Reward(OfferID, EngineID, CreateROID, ProgramID, Quantity, Multiplier, Phase, ScorecardID, ScorecardDesc, ScorecardBold, MaxAdjustmentEnabled, MaxAdjustment, ChargebackDeptID, RewardRequired, DeliverableID)) Then
                        ' Insert tier values
                        MyCommon.QueryStr = "select PKID from CPE_DeliverablePoints with (NoLock) where DeliverableID=" & DeliverableID & ";"
                        rst = MyCommon.LRT_Select()
                        If rst.Rows.Count > 0 Then
                ...
                Else
					...
					' vtopol
					If (Update_Reward(OfferID, EngineID, CreateROID, DeliverableID, ProgramID, Quantity, Multiplier, Phase, ScorecardID, ScorecardDesc, ScorecardBold, MaxAdjustmentEnabled, MaxAdjustment, ChargebackDeptID, RewardRequired)) Then
...
<%
  ...
  MyCommon.QueryStr = "Select ProgramID, ProgramName from PointsPrograms with (NoLock) " & _
                      "where Deleted=0 and CAMProgram=0 and ProgramID is not null " & _
                      "  and ProgramID not in  " & _
                      "    (select ProgramID from CPE_DeliverablePoints with (NoLock) " & _
                      "     where RewardOptionID=" & RewardID & " and Deleted=0 and DeliverableID <> " & DeliverableID & ") " & _
                      IIf(IsAnyCustomer, " and ProgramID in (SELECT ProgramID FROM PointsProgramsPromoEngineSettings WHERE AllowAnyCustomer = 1)","" ) & _ 
...
function toggleScorecardText() {
  if (document.getElementById("ScorecardID") != null && document.getElementById("ScorecardDescLine") != null && document.getElementById("ScorecardDesc") != null) {
    ...
    } else if(document.getElementById("ScorecardDescLine") != null) {
...
function generateSCInputs(ProgramID) {
  xmlhttpPost('/logix/ScorecardFeeds.aspx', 'ScorecardFieldsForReward=1&EngineID=<%Sendb(EngineID)%>&OfferID=<%Sendb(OfferID)%>&DeliverableID=<%Sendb(DeliverableID)%>&ProgramID=' + ProgramID + '&ScorecardTypeID=1');
  toggleScorecardText();
}
...
<%
  ...
  End If
%>
<form action="UEoffer-rew-point.aspx" id="mainform" name="mainform" onsubmit="return validateEntry();">
  <div id="intro">
      ...
      <div class="box" id="distribution">
        ...
        <table summary="<% Sendb(Copient.PhraseLib.Lookup("term.distribution", LanguageID))%>">
          <tr>
            ...
            <td>
              <%
                If DeliverableID = 0 OrElse ReloadFromBrowser Then
                  For t = 1 To TierLevels

                    If (SupportGlobalAndTieredConditions = 1 and t = 1) Then
                      Send("  <input type=""checkbox"" name=""useSameTierValue"" id =""useSameTierValue"" align='top' value=""1""" & IIf(UseSameTierValue = 1, " checked=""checked""", "") & " style=""margin-left:7px;"" onclick=""setSameTierValue(" & TierLevels & ")""/>")
                    ...
                    If Request.QueryString("t" & t & "_quantity") <> "" Then
                      ...
                      End If
                              Sendb("<input type=""text"" class=""shorter"" id=""t" & t & "_quantity"" name=""t" & t & "_quantity"" maxlength=""6"" value=""" & Request.QueryString("t" & t & "_quantity") & """/>")
                    Else
                      If TierLevels > 1 Then
                        Send("&nbsp;&nbsp;<label for=""t" & t & "_quantity"">" & Copient.PhraseLib.Lookup("term.tier", LanguageID) & " " & t & ":</label>")
                      End If
                  ...
                  Next
                Else
				 MyCommon.QueryStr = "Select TierLevel, Quantity, Multiplier  from CPE_DeliverablePointTiers with (NoLock) " & _
                  ...
                  For t = 1 To TierLevels
                    ...
                    If t > rst.Rows.Count Then
                      Sendb(" value=""0""" & IIf(UseSameTierValue = 1, " disabled=""disabled""", DisabledAttribute) & " />")
                    Else
...
<script runat="server">
  Function Create_Reward(ByVal OfferID As Long, ByVal EngineID As Long, ByVal ROID As Long, ByVal ProgramID As Long, ByVal Quantity As Long, ByVal Multiplier As Decimal, _
                         ByVal Phase As Long, ByVal ScorecardID As Long, ByVal ScorecardDesc As String, ByVal ScorecardBold As Boolean, _
                         ByVal MaxAdjustmentEnabled As Boolean, ByVal MaxAdjustment As Long, ByVal ChargebackDeptID As Integer, _
    ...
    Try
	  ...
	  MyCommon.LRTsp.Parameters.Add("@Multiplier", SqlDbType.Decimal, 9).Value = Multiplier	  
  ...
  Function Update_Reward(ByVal OfferID As Long, ByVal EngineID As Long, ByVal ROID As Long, ByVal DeliverableID As Long, ByVal ProgramID As Long, _
						 ByVal Quantity As Long, ByVal Multiplier As Decimal, ByVal Phase As Long, ByVal ScorecardID As Long, ByVal ScorecardDesc As String, ByVal ScorecardBold As Boolean, _
                         ByVal MaxAdjustmentEnabled As Boolean, ByVal MaxAdjustment As Long, ByVal ChargebackDeptID As Integer, ByVal RewardRequired As Boolean) As Boolean
    Dim MyCommon As New Copient.CommonInc
    Dim Status As Integer = 0
  ...
  Sub Create_RewardTiers(ByVal DPPKID As Long, ByVal TierLevel As Long, ByVal Quantity As Decimal)
    Dim MyCommon As New Copient.CommonInc
...
<script type="text/javascript">
    function setSameTierValue(tierLevels){
      ...
      if(box.checked){
        for (i=1; i < (tierLevels + 1); i++){
          text = "t" + i.toString() + "_quantity";
          //alert(document.getElementById("tier1_l1discountamt").value.toString());
          document.getElementById(text).value = document.getElementById("t1_quantity").value;
    ...
    function enableTiers(){
      var t = 1
      qtyElem = document.getElementById("t" + t + "_quantity");
...
<% If (CloseAfterSave) Then %>
  window.location = '/logix/close.aspx';
<% End If %>
</script>
<%
  If (ProgramID > 0) Then
