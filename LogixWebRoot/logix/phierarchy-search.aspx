...
<%
  ' *****************************************************************************
  ' * FILENAME: phierarchy-search.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Response.Expires = 0
  MyCommon.AppName = "phierarchy-search.aspx"
  ...
  If (SelectedOption <> "" AndAlso Request.QueryString("Find") = "") Then
    ...
    If (levelPos > -1 AndAlso idPos > -1) Then
      ...
      Long.TryParse(SelectedOption.Substring(idPos + 2), ID)
  ...
  Send_HeadBegin("term.offer", "term.producthierarchy")
...
<%  If (qryStr <> "") Then %>
        if (opener != null && opener.document != null) {
            ...
            if (pageName.indexOf("phierarchy.aspx") > -1) {
                pageName = 'phierarchy.aspx?<% Sendb(qryStr) %>'
            } else if (pageName.indexOf("pgroup-edit.aspx") > -1) {
                var pg = '<%Sendb(ProductGroupID)%>'
                if (pg == "") {
                    var pgStart = pageName.indexOf("ProductGroupID=");
                    var pgEnd = pageName.indexOf("&", pgStart);
                    if (pgStart > -1) {
                        if (pgEnd > -1) {
                            pg = pageName.substring(pgStart + "ProductGroupID".length + 1, pgEnd);
                        } else {
                            pg = pageName.substring(pgStart + "ProductGroupID".length + 1);
                        }
                    }
                }
                pageName = 'pgroup-edit.aspx?ProductGroupID=' + pg + '&<% Sendb(qryStr) %>';
            } else {
                pageName = 'phierarchy.aspx?<% Sendb(qryStr) %>'
            }
...
<% 
  ...
  End If
%>
<form action="phierarchy-search.aspx" id="mainform" name="mainform">
  <input type="hidden" name="ProductGroupID" id="ProductGroupID" value="<%Sendb(ProductGroupID) %>" />
  <div id="intro">
    <h1 id="title">
    ...
    <div id="column">
      <div class="box" id="selector">
        ...
        <div style="float: right; text-align: right;">
          <input type="button" class="regular" id="select" name="select" value="<% Sendb(Copient.PhraseLib.Lookup("term.select", LanguageID)) %>" onclick="select_Click();" />
          <input type="button" class="regular" id="close" name="close" value="<% Sendb(Copient.PhraseLib.Lookup("term.close", LanguageID)) %>" onclick="close_Click();" />
        </div>
        <br clear="all" />
        <br />
            ...
            <tbody>
              <%
                If (SearchString <> "") Then
                  MyCommon.QueryStr = "dbo.pa_Product_Hierarchy_Search"
                  MyCommon.Open_LRTsp()
                  ...
                  If (rst.Rows.Count > 0) Then
                    ...
                    Next
                  Else
                    Send("<tr><td colspan=""4""><center><i>" & Copient.PhraseLib.Lookup("phierarchy.noresultsfound", LanguageID) & " '" & SearchString & "'</i></center></td></tr>")
                  End If
                End If
              %>
            </tbody>
...
<script runat="server">
  ...
  Function GetHierarchyID(ByVal NodeId As Long) As Long
  ...
  Function GetParentNode(ByVal NodeId As Long) As Long
  ...
  Function GetItemNodeID(ByVal PKID As Long) As Long
