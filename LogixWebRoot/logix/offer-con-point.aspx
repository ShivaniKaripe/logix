...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "offer-con-point.aspx"
...
function removeUsed() {
  xmlhttpPost('OfferFeeds.aspx', 'PointsProgramsCM');
...
function validateEntry() {
  ...
  var qtyElem = document.getElementById("tier0");
  var numtiers = parseInt(document.getElementById("NumTiers").value);
  ...
  var msg = '';
    ...
    {
    qtyElem = document.getElementById('tier'+i.toString());
      if (qtyElem != null) {
        // trim the string
        qtyVal = qtyElem.value.replace(/^\s+|\s+$/g, ''); 
...
function handleCreateClick(createbtn)
 ...
 function xmlhttpPost_CreateGroupOrProgramFromOffer(strURL,mode) {
    ...
    {
        return "Mode=" + mode + "&CreateType=Points&Name=" + document.getElementById('functioninput').value
    }
...
<%
  ...
  If (Request.QueryString("save") <> "") Then
    ...
    MyCommon.LRT_Execute()
    MyCommon.Activity_Log(3, OfferID, AdminUserID, Copient.PhraseLib.Lookup("history.con-point", LanguageID))
    If (infoMessage = "") Then
  ...
  <div id="main">
    ...
    <div id="column1">
      <div class="box" id="selector">
        ...
        <input class="medium" onkeyup="javascript:xmlPostTimer('OfferFeeds.aspx','PointsProgramsCM');" id="functioninput" name="functioninput" type="text" maxlength="100" value=""<% sendb(disabledattribute) %> />
        ...
        <select class="longer" id="functionselect" name="functionselect" size="16"<% sendb(disabledattribute) %>>
          <%
              ...
              MyCommon.QueryStr = "Select " & topString & "PP.ProgramID,PP.ProgramName from PointsPrograms as PP with (NoLock) where deleted=0 and ProgramID is not null order by ProgramName"
            rst = MyCommon.LRT_Select
        ...
        <select class="longer" id="selected" name="selected" size="2"<% sendb(disabledattribute) %>>
          <%
            MyCommon.QueryStr = "select PP.ProgramID,PP.ProgramName,OC.LinkID from PointsPrograms as PP with (NoLock) left join OfferConditions as OC with (NoLock) on OC.LinkID=PP.ProgramID where OC.ConditionID=" & ConditionID
            rst = MyCommon.LRT_Select
            For Each row In rst.Rows
              Send("<option value=""" & MyCommon.NZ(row.Item("ProgramID"), -1) & """>" & MyCommon.NZ(row.Item("ProgramName"), "") & "</option>")
            Next
      ...
      <div class="box" id="value">
        ...
        <%
          ...
          For Each row In rst.Rows
            ...
            q = q + 1
          Next
          Send("<input type=""hidden"" id=""NumTiers"" name=""NumTiers"" value=""" & MyCommon.NZ(row.Item("NumTiers"), "") & """ />")
        ...
        <label for="RadioValue1"><% Sendb(Copient.PhraseLib.Lookup("condition.PreviousValue", LanguageID))%></label>
        <br />
        ...
        <label for="RadioValue2"><% Sendb(Copient.PhraseLib.Lookup("condition.CurrentValue", LanguageID))%></label>
        <br />
