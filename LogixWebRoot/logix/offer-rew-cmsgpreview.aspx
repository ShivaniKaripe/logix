...
<%
  ' *****************************************************************************
  ' * FILENAME: offer-rew-cmsgpreview.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  {"\|SVVALNET\[[0-9]+\]\|", "\|NET\#[[0-9]+]\|", "\|EARNED\#[[0-9]+]\|", "\|INITIAL#\[[0-9]+]\|", "\|REDEEMED#\[[0-9]+]\|", "\|NET\$\[[0-9]+]\|", "\|INITIAL\$\[[0-9]+]\|", "\|EARNED\$\[[0-9]+]\|", "\|REDEEMED\$\[[0-9]+]\|", "\|SVBAL\[[0-9]+\]\|", "\|SVVAL\[[0-9]+\]\|", "\|UPCA\[[0-9]+\]\|", "\|UPCB\[[0-9]+\]\|", "\|SCORECARD\[[0-9]+\]\|", "\|SVRATIO\[[0-9]+\,[0-9]+\,[0-9,.]+\]\|", "\|PTBAL\[[0-9]+\]\|"}, _
  {"###.##", "###", "###", "###", "###", "###.##", "###.##", "###.##", "###.##", "####", "###.##", "<img src='../images/upca.png'>", "<img src='../images/upcb.png'>", "**************SCORECARD**************", "###.##", "####"}}
  
  Response.Expires = 0
  MyCommon.AppName = "offer-rew-cmsgpreview.aspx"
  ...
  RawMessage = Request.QueryString("Line1") + vbLf + vbLf + Request.QueryString("Line2")
...
<script runat="server">
  ...
  Function LineWrap(ByVal wrapLength As Integer, ByVal NewLine As String) As String
    ...
    If NewBRLine.Length > wrapLength Then
      ...
      y = 0
	  If (NewBRLine.Length > 0 And y < 100) Then
                y = y + 1
                While (NewBRLine.Length > wrapLength)
                    FormatLine = FormatLine & NewBRLine.Substring(0, wrapLength)
                    NewBRLine = NewBRLine.Remove(0, wrapLength)
                    If (NewBRLine.Length > wrapLength) Then
                        If (NewBRLine.Substring(0, wrapLength).StartsWith("|")) Then
                            FormatLine = FormatLine & "<br/>"
                        Else
                            Dim idxDlm As Integer = NewBRLine.IndexOf("|")
                            FormatLine = FormatLine & NewBRLine.Substring(0, idxDlm + 1) & "<br/>"
                            NewBRLine = NewBRLine.Remove(0, idxDlm + 1)
                        End If
                    Else
                        If Not (NewBRLine.StartsWith("|")) Then
                            Dim idxDlm As Integer = NewBRLine.IndexOf("|")
                            FormatLine = FormatLine & NewBRLine.Substring(0, idxDlm + 1) & "<br/>"
                            NewBRLine = NewBRLine.Remove(0, idxDlm + 1)
                        End If
