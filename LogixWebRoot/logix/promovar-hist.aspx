...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "promovar-hist.aspx"
  ...
  If (ProgramGroupID = 0) Then
    ...
    Response.AddHeader("Location", "point-edit.aspx?new=New")
  End If
  
  MyCommon.QueryStr = "select ProgramName, Deleted from PointsPrograms with (NoLock) where ProgramID=" & ProgramGroupID & ";"
  ...
  Send_HeadBegin("term.promovar", "term.history", l_pgID)
...
<div id="intro">
  <h1 id="title">
    <%
      Sendb(Copient.PhraseLib.Lookup("term.promovar", LanguageID) & " #" & ProgramGroupID)
      If GName <> "" Then
    ...
    <%
      If MyCommon.Fetch_SystemOption(75) Then
        If (l_pgID > 0 And Logix.UserRoles.AccessNotes) Then
          Send_NotesButton(10, l_pgID, AdminUserID)
        End If
      End If
    %>
  </div>
...
<div id="main">
  ...
  <table class="list" summary="<% Sendb(Copient.PhraseLib.Lookup("term.history", LanguageID)) %>">
    ...
    <tbody>
      <%
        MyCommon.QueryStr = "select AU.FirstName, AU.LastName, AL.ActivityDate, AL.Description from ActivityLog as AL with (NoLock) left join AdminUsers as AU with (NoLock) on AL.AdminID=AU.AdminUserID Where ActivityTypeID='7' and LinkID='" & ProgramGroupID & "' order by ActivityDate desc;"
...
<%
  If MyCommon.Fetch_SystemOption(75) Then
    If (l_pgID > 0 And Logix.UserRoles.AccessNotes) Then
      Send_Notes(10, l_pgID, AdminUserID)
    End If
  End If
done:
  Send_BodyEnd()
