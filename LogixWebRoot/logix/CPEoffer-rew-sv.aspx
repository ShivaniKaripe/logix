...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "CPEoffer-rew-sv.aspx"
  ...
  If (Single.TryParse(QuantityDisplay, Quantity)) Then
    Quantity = CInt(MyCommon.Extract_Val(QuantityDisplay))
  ...
  If (Request.QueryString("save") <> "") Then
    ...
    If ValidTiers Then
      ...
      If (OfferID > 0 AndAlso RewardID > 0 AndAlso ProgramID > 0 AndAlso DeliverableID = 0) Then
        ...
        If (Create_Reward(OfferID, CreateROID, ProgramID, Quantity, Phase, ScorecardID, ScorecardDesc, ScorecardBold, MaxAdjustment, DeliverableID)) Then
...
<%
  ...
  Else
    MyCommon.querystr = "Select SVProgramID as ProgramID,Name as ProgramName from StoredValuePrograms with (NoLock) " & _
                        "where deleted=0 and SVPRogramID Not in  " & _
                        "( select SVProgramID from CPE_Deliverables D with (NoLock)  " & _
                        "  inner join CPE_DeliverableStoredValue DSV with (NoLock) on DSV.PKID = D.OutputID " & _
                        "  where D.Deleted=0 and D.RewardOptionID = " & RewardID & " and D.DeliverableTypeID=11 " & _
                        "     and D.DeliverableID <> " & DeliverableID & " and DSV.Deleted=0) " & _
...
function generateSCInputs(ProgramID) {
  xmlhttpPost('ScorecardFeeds.aspx', 'ScorecardFieldsForReward=1&EngineID=<%Sendb(EngineID)%>&OfferID=<%Sendb(OfferID)%>&DeliverableID=<%Sendb(DeliverableID)%>&ProgramID=' + ProgramID + '&ScorecardTypeID=2');
  toggleScorecardText();
}
...
<%
  ...
  End If
%>
<form action="CPEoffer-rew-sv.aspx" id="mainform" name="mainform" onsubmit="return validateEntry();">
  <div id="intro">
    ...
    <div id="column2">
      <div class="box" id="distribution">
        ...
        <%
          ...
          MyCommon.QueryStr = "Select TierLevel, Quantity from CPE_DeliverableStoredValueTiers with (NoLock) " & _
  ...
  <script runat="server">
    Function Create_Reward(ByVal OfferID As Long, ByVal ROID As Long, ByVal ProgramID As Long, ByVal Quantity As Decimal, ByVal Phase As Long, ByVal ScorecardID As Long, ByVal ScorecardDesc As String, ByVal ScorecardBold As Boolean, ByVal MaxAdjustment As Long, ByRef DeliverableID As Long) As Boolean
      Dim MyCommon As New Copient.CommonInc
      ...
      Try
        ...
        MyCommon.LRTsp.Parameters.Add("@Quantity", SqlDbType.Decimal).Value = Quantity
    ...
    Sub Create_RewardTiers(ByVal DSVPKID As Long, ByVal TierLevel As Long, ByVal Quantity As Long)
      Dim MyCommon As New Copient.CommonInc
