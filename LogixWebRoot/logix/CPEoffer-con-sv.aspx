...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "CPEoffer-con-sv.aspx"
  ...
  MyCommon.QueryStr = "select IncentiveStoredValueID, SVProgramID from CPE_IncentiveStoredValuePrograms where RewardOptionID=" & roid & " and Deleted=0;"
  rst = MyCommon.LRT_Select
  ...
  If (Request.QueryString("save") <> "" And roid > 0) Then
    ...
    If ValidTier Then
      ...
      MyCommon.QueryStr = "delete from CPE_IncentiveStoredValueProgramTiers where RewardOptionID=" & roid & ";"
      MyCommon.LRT_Execute()
      
      If (Request.QueryString("selGroups") <> "") Then
        Dim QtyString As String = ""
        If TierLevels = 1 Then
          QtyString = Copient.PhraseLib.Detokenize("CPEoffer-con-sv.RequiresSV", LanguageID, Request.QueryString("t1_QtyForIncentive"))
        Else
          For t = 1 To TierLevels
            QtyString += Copient.PhraseLib.Detokenize("CPEoffer-con-sv.TierRequiresSV", LanguageID, t, Request.QueryString("t" & t & "_QtyForIncentive"))
          Next
        End If
        historyString = Copient.PhraseLib.Detokenize("CPEoffer-con-sv.AlteredGroup", LanguageID, Request.QueryString("selGroups") & " " & QtyString)
        'Check for SV program to update
        MyCommon.QueryStr = "select IncentiveStoredValueID from CPE_IncentiveStoredValuePrograms with (NoLock) where RewardOptionID=" & roid & " and Deleted=0;"
        rst = MyCommon.LRT_Select()
        If rst.Rows.Count = 0 Then
        ...
        Else
          MyCommon.QueryStr = "update CPE_IncentiveStoredValuePrograms with (RowLock) set SVProgramID=" & SVID & ",QtyForIncentive=" & MyCommon.NZ(Request.QueryString("t1_QtyForIncentive"), 0) & "," & _
                              "Deleted=0,LastUpdate=getdate() where RewardOptionID=" & roid & " and Deleted=0;"
          MyCommon.LRT_Execute()
        End If
        MyCommon.QueryStr = "select IncentiveStoredValueID from CPE_IncentiveStoredValuePrograms where RewardOptionID=" & roid & " and Deleted=0;"
        rst = MyCommon.LRT_Select()
        If rst.Rows.Count > 0 Then
        ...
        <select class="longer" id="selected" name="selected" size="2"<% sendb(disabledattribute) %>>
          <%
            If IncentiveSVID = 0 Then
              ...
              End If
            Else
              MyCommon.QueryStr = "Select IPG.SVProgramID as ProgramID,Name as ProgramName from CPE_IncentiveStoredValuePrograms as IPG with (NoLock) left join StoredValuePrograms as PP with (NoLock) on PP.SVProgramID=IPG.SVProgramID where IPG.deleted=0 and RewardOptionID=" & roid
              rst = MyCommon.LRT_Select
              For Each row In rst.Rows
                Send("<option value=""" & MyCommon.NZ(row.Item("ProgramID"), 0) & """>" & MyCommon.NZ(row.Item("ProgramName"), "") & "</option>")
              Next
    ...
    <div id="column2">
      <div class="box" id="value">
        ...
        <label for="t1_QtyForIncentive">
          <% Sendb(Copient.PhraseLib.Lookup("condition.quantityneeded", LanguageID))%>
        </label>
        <br />
        <%
          If TierLevels > 1 Then
          ...
          Else
            ...
            Else
              ...
              MyCommon.QueryStr = "Select QtyForIncentive from CPE_IncentiveStoredValuePrograms with (NoLock) where deleted=0 and RewardOptionID=" & roid
              rst = MyCommon.LRT_Select
              If (rst.Rows.Count > 0) Then
