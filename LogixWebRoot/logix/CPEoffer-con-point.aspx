...
<%
  ' *****************************************************************************
  ' * FILENAME: CPEoffer-con-point.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Response.Expires = 0
  MyCommon.AppName = "CPEoffer-con-point.aspx"
  ...
  MyCommon.QueryStr = "select ProgramID, IncentivePointsID from CPE_IncentivePointsGroups with (NoLock) where RewardOptionID=" & roid & " and Deleted=0;"
  rst = MyCommon.LRT_Select
  ...
  If (Request.QueryString("save") <> "" And roid > 0) Then
    ...
    If ValidTier Then
      ...
      MyCommon.QueryStr = "delete from CPE_IncentivePointsGroupTiers where RewardOptionID=" & roid & ";"
      MyCommon.LRT_Execute()
      
      ' check to see if a points condition is required by the template, if applicable
      MyCommon.QueryStr = "select ProgramID from CPE_IncentivePointsGroups with (NoLock) where RewardOptionID=" & roid & _
                          " and RequiredFromTemplate=1 and Deleted=0;"
      rst = MyCommon.LRT_Select
      HasRequiredPP = (rst.Rows.Count > 0)
      'Check for points condition to update
      MyCommon.QueryStr = "select IncentivePointsID from CPE_IncentivePointsGroups where Deleted=0 and RewardOptionID=" & roid & ";"
      rst = MyCommon.LRT_Select()
      Dim QtyString As String = ""
      If TierLevels = 1 Then
        QtyString = Copient.PhraseLib.Detokenize("CPEoffer-con-point.RequiresPoints", LanguageID, Request.QueryString("t1_QtyForIncentive"))
      Else
        For t = 1 To TierLevels
          QtyString += " TierLevel " & t & " requires " & Request.QueryString("t" & t & "_QtyForIncentive") & " points;"
          QtyString = Copient.PhraseLib.Detokenize("CPEoffer-con-point.TierRequiresPoints", LanguageID, t, Request.QueryString("t" & t & "_QtyForIncentive"))
        Next
      End If
      historyString = Copient.PhraseLib.Detokenize("CPEoffer-con-point.AlteredGroup", LanguageID, Request.QueryString("selGroups") & " " & QtyString & " ")
      If rst.Rows.Count > 0 Then
        MyCommon.QueryStr = "update CPE_IncentivePointsGroups set ProgramID=" & ProgramID & ", QtyForIncentive=" & MyCommon.NZ(Request.QueryString("t1_QtyForIncentive"), 0) & ", " & _
                            "Deleted=0, LastUpdate=getdate(), RequiredFromTemplate=" & IIf(HasRequiredPP, "1", "0") & " where RewardOptionID=" & roid & ";"
        MyCommon.LRT_Execute()
      Else
        ...
        End If
      End If
      MyCommon.QueryStr = "select IncentivePointsID from CPE_IncentivePointsGroups where RewardOptionID=" & roid & " and Deleted=0;"
      rst = MyCommon.LRT_Select()
      If rst.Rows.Count > 0 Then
    ...
    <div id="column1">
      <div class="box" id="selector">
        ...
        <select class="longer" id="selected" name="selected" size="2"<% sendb(disabledattribute) %>>
          <%
            If PointsID <> 0 Then
              MyCommon.QueryStr = "Select IPG.ProgramID,ProgramName from CPE_IncentivePointsGroups as IPG with (NoLock) left join PointsPrograms as PP with (NoLock) on PP.ProgramID=IPG.ProgramID where IPG.deleted=0 and IPG.ProgramID is not null and RewardOptionID=" & roid
              rst = MyCommon.LRT_Select
              For Each row In rst.Rows
                Send("<option value=""" & MyCommon.NZ(row.Item("ProgramID"), 0) & """>" & MyCommon.NZ(row.Item("ProgramName"), "") & "</option>")
              Next
        ...
        <%
            ...
            If PointsID = 0 Then
              For t = 1 To TierLevels
                If Request.QueryString("t" & t & "_QtyForIncentive") <> "" Then
                  ...
                  Send("        <input type=""text"" class=""short"" id=""t" & t & "_QtyForIncentive"" name=""t" & t & "_QtyForIncentive"" value=""" & MyCommon.Extract_Val(Request.QueryString("t" & t & "_QtyForIncentive")) & """ " & DisabledAttribute & " maxlength=""9"" /><br />")
                Else
                  Send("        <label for=""t" & t & "_QtyForIncentive"">" & Copient.PhraseLib.Lookup("term.tier", LanguageID) & " " & t & "</label>")
          ...
          Else
            If PointsID = 0 Then
              ...
              End If
            Else
              MyCommon.QueryStr = "Select QtyForIncentive from CPE_IncentivePointsGroups with (NoLock) where deleted=0 and RewardOptionID=" & roid
              rst = MyCommon.LRT_Select
              If (rst.Rows.Count > 0) Then
