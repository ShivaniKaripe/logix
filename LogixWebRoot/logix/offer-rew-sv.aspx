...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "offer-rew-sv.aspx"
  ...
  Else
    MyCommon.QueryStr = "select rewardid from offerrewards with (NoLock) where offerid=" & OfferID & " and rewardtypeid=10"
    rst3 = MyCommon.LRT_Select
    If rst3.Rows.Count = 1 Then
      GName = OfferStartDate.ToString("yyyyMMdd") & Name & "PGRSV"
    ElseIf rst3.Rows.Count > 1 Then
      GName = OfferStartDate.ToString("yyyyMMdd") & Name & "PGRSV(" & rst3.Rows.Count - 1 & ")"
    End If
  End If
...
function getproductquery(mode) {
  ...
  return "Mode=" + mode + "&ProductSearch=" + document.getElementById('functioninput').value + "&OfferID=" + document.getElementById('OfferID').value + "&SelectedGroup=" + selectedGroup + "&ExcludedGroup=" + excludedGroup + "&SearchRadio=" + radioString +  "&CallingPage=rewStoredValue";
...
{
    xmlhttpPostPP('OfferFeeds.aspx','StoredValueProgramsCM');
    // this function will remove items from the functionselect box that are used in 
    // selected and excluded boxes
...
function PhandleSelectClick(itemSelected)
{
  ...
  if (itemSelected == "Pselect1") {
    if (selectedValue != ""){
      ...
      document.getElementById("SVTypeID").value = document.getElementById("Pfunctionselect").value;
...
function handlePointsCreateClick(createbtn)
 ...
 function xmlhttpPost_CreatePointProgramFromOffer(strURL,mode) {
    ...
    {
        return "Mode=" + mode + "&CreateType=StoredValue&Name=" + document.getElementById('Pfunctioninput').value
    }
...
<%
  ...
  MyCommon.QueryStr = "select RewardStoredValuesID,ProgramID,ChargeBackDeptID,DiscountableItemsOnly,PrintLineText, WebText, SVTypeID from CM_RewardStoredValues with (NoLock) where RewardStoredValuesID=" & LinkID
  rst = MyCommon.LRT_Select()
  For Each row In rst.Rows
    ...
    SVTypeID = MyCommon.NZ(row.Item("SVTypeID"), 0)
  Next
  ...
  ElseIf (Request.QueryString("save") <> "" Or Request.QueryString("clicksent") = "true" Or _
    ...
    If Not (bUseTemplateLocks And bDisallowEditAdv) Then
      ...
      If (Request.QueryString("disc-items-only") = "on") Then
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set DiscountableItemsOnly=1 where RewardStoredValuesID=" & LinkID
        MyCommon.LRT_Execute()
        DiscountableItemsOnly = True
      Else
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set DiscountableItemsOnly=0 where RewardStoredValuesID=" & LinkID
        MyCommon.LRT_Execute()
    ...
    If Not (bUseTemplateLocks And bDisallowEditLimit) Then
      ...
      If AdvancedLimitID = 0 Then
        ...
        If (Request.QueryString("limitvalue") <> "") Then
          If (Request.QueryString("SVTypeID") = "1" And intNumDecimalPlaces > 0) Then
            bNeedToFormat = True
          Else
            bNeedToFormat = False
          End If
          If bNeedToFormat Then
            decTemp = MyCommon.Extract_Val(Request.QueryString("limitvalue")) * decFactor
          Else
            decTemp = MyCommon.Extract_Val(Request.QueryString("limitvalue"))
          End If
    ...
    If Not (bUseTemplateLocks And bDisallowEditMsg) Then
      If (GetCgiValue("PrintLineText") <> "") Then
        ...
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set PrintLineText=@PrintLineText where RewardStoredValuesID=@RewardStoredValuesID"
        MyCommon.DBParameters.Add("@PrintLineText", SqlDbType.NVarChar, 1000).Value = PrintLineText
        MyCommon.DBParameters.Add("@RewardStoredValuesID", SqlDbType.BigInt).Value = LinkID
        MyCommon.ExecuteNonQuery(Copient.DataBases.LogixRT)
      Else
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set PrintLineText=null where RewardStoredValuesID=" & LinkID
        MyCommon.LRT_Execute()
      End If
      If (GetCgiValue("WebText") <> "") Then
        WebText = GetCgiValue("WebText")
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set WebText=@WebText where RewardStoredValuesID=@RewardStoredValuesID"
        MyCommon.DBParameters.Add("@WebText", SqlDbType.NVarChar, 1000).Value = PrintLineText
        MyCommon.DBParameters.Add("@RewardStoredValuesID", SqlDbType.BigInt).Value = LinkID
        MyCommon.ExecuteNonQuery(Copient.DataBases.LogixRT)
      Else
        MyCommon.QueryStr = "update CM_RewardStoredValues with (RowLock) set WebText=null where RewardStoredValuesID=" & LinkID
        MyCommon.LRT_Execute()
      End If
    End If
      ...
      Else
        ...
        If Not (bUseTemplateLocks And bDisallowEditDist) Then
          ...
          If (Tiered = 0) Then
            ...
            MyCommon.LRTsp.Parameters.Add("@RewardAmount", SqlDbType.Decimal).Value = decTemp
            MyCommon.LRTsp.ExecuteNonQuery()
            MyCommon.Close_LRTsp()
          Else
    ...
    MyCommon.LRT_Execute()
        ...
        MyCommon.Activity_Log(3, OfferID, AdminUserID, Copient.PhraseLib.Lookup("history.rew-sv", LanguageID))
...
<form action="offer-rew-sv.aspx" id="mainform" name="mainform" onkeydown="if (event.keyCode == 13) return false;" onsubmit="return saveForm();">
<div id="intro">
...
<%Send("<div id=""infobar1"" >")
  ...
  <% If (infoMessage <> "") Then Send("<div id=""infobar"" class=""red-background"">" & infoMessage & "</div>")%>
    ...
    <div class="box" id="message">
      ...
      <div id="divWebText">
        ...
        <label for="WebText" style="position: relative;">
          <% Sendb(Copient.PhraseLib.Lookup("term.webmessage", LanguageID))%>
          :</label>
    ...
    <div class="box" id="limits" style="position: relative;">
      ...
      <%
        If (SVTypeID = "1") Then
          decTemp = (RewardLimit * 1.0) / decFactor
          sTemp = FormatNumber(decTemp, intNumDecimalPlaces)
        Else
          sTemp = RewardLimit.ToString()
        End If
      %>
      <input class="shorter" id="limitvalue" name="limitvalue" maxlength="9" type="text"
        value="<% sendb(sTemp) %>" <% If(bUseTemplateLocks and bDisallowEditLimit) Then sendb(" disabled=""disabled""") %> />
      <% Sendb(Copient.PhraseLib.Lookup("term.point(s)", LanguageID))%>
      &nbsp;<% Sendb(Copient.PhraseLib.Lookup("term.per", LanguageID))%>
      <input class="shorter" id="limitperiod" name="form_DistPeriod" maxlength="4" type="text"
        value="<% sendb(DistPeriod) %>" <% If(bUseTemplateLocks and bDisallowEditLimit) Then sendb(" disabled=""disabled""") %> />
    ...
    <div id="column2">
      ...
      <input class="medium" onkeyup="javascript:xmlPostTimerPP('OfferFeeds.aspx','StoredValueProgramsCM');" id="Pfunctioninput" name="Pfunctioninput"
          type="text" maxlength="100" value="" <% sendb(disabledattribute) %> />
        ...
        <% End If%>
        <br />
        <div id="searchLoadDivPp" style="display: block;">
        &nbsp;</div>
        <div id="ppList">
        <select class="longer" id="Pfunctionselect" name="Pfunctionselect" size="10" <% sendb(disabledattribute) %>>
          <%
          MyCommon.QueryStr = "select " & topString & " SVProgramId as ProgramID,Name as ProgramName from StoredValuePrograms with (NoLock) where Deleted=0 and SVProgramID is not null and SVTypeID not in (4,5) order by ProgramName"
            rst2 = MyCommon.LRT_Select
      ...
      %>
        ...
        <%
          ...
          For Each row2 In rst2.Rows
            Send("<option value=""" & row2.Item("ProgramID") & """>" & row2.Item("ProgramName") & "</option>")
            'SVTypeID = Int(MyCommon.NZ(row2.Item("SVTypeID"), 0))
            If (Request.QueryString("SVTypeID") = "1" And intNumDecimalPlaces > 0) Then
              bNeedToFormat = True
            Else
              bNeedToFormat = False
          ...
          For Each row In rst.Rows
            ...
            If (row.Item("UseSpecialPricing") <> True) Then
              If (SVTypeID = "1") Then
                ...
                sTemp = FormatNumber(decTemp, intNumDecimalPlaces)
              Else
                sTemp = Int(MyCommon.NZ(row.Item("RewardAmount"), 0)).ToString()
              End If
              If (row.Item("Tiered") = False) Then
                Sendb("<input type=""text"" onkeypress=""return submitenter(event)"" class=""shorter"" id=""tier0"" name=""tier0"" style=""width:45px;float:left;"" maxlength=""9"" value=""" & sTemp & """" & sDisabled & " />")
              Else
                Sendb("<label for=""tier" & q & """>" & Copient.PhraseLib.Lookup("term.tier", LanguageID) & q & ":</label> <input class=""shorter"" id=""tier" & q & """ name=""tier" & q & """ type=""text"" style=""width:45px;"" maxlength=""9"" value=""" & sTemp & """" & sDisabled & " /><br />")
              End If
            End If
            q = q + 1
          Next
          Send("</td>")
          Send("<td id=""perspan"" style=""width:30px;"">" & Copient.PhraseLib.Lookup("term.per", LanguageID) & "</td>")
          Send("<td>")
