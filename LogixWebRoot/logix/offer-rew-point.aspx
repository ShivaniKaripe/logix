...
<%
  ...
  Response.Expires = 0
  MyCommon.AppName = "offer-rew-point.aspx"
  ...
  Else
    MyCommon.QueryStr = "select rewardid from offerrewards with (NoLock) where offerid=" & OfferID & " and rewardtypeid=2"
    rst3 = MyCommon.LRT_Select
    If rst3.Rows.Count = 1 Then
      GName = OfferStartDate.ToString("yyyyMMdd") & Name & "PGRP"
    ElseIf rst3.Rows.Count > 1 Then
      GName = OfferStartDate.ToString("yyyyMMdd") & Name & "PGRP(" & rst3.Rows.Count - 1 & ")"
    End If
  End If
...
//to load point programs based on search criteria.
var Ptimer;
function xmlPostTimerPP(strURL,mode)
{
  clearTimeout(Ptimer);
  Ptimer=setTimeout("xmlhttpPostPP('" + strURL + "','" + mode + "')", 250);
}
...
function xmlhttpPostPP(strURL,mode) {
  var xmlHttpReq = false;
  var self = this;
  document.getElementById("searchLoadDivPp").innerHTML = '<% Sendb(Copient.PhraseLib.Lookup("message.loading", LanguageID)) %>';
  if (window.XMLHttpRequest) {
    self.xmlHttpReq = new XMLHttpRequest();
  }
  // IE
  ...
  self.xmlHttpReq.onreadystatechange = function() {
    if (self.xmlHttpReq !=null && self.xmlHttpReq.readyState == 4) {
      updatepagePP(self.xmlHttpReq.responseText);
    }
    }
...
function getprogramquery(mode) {
  var radioString;
  if(document.getElementById('Pfunctionradio2').checked) {
    radioString = 'Pfunctionradio2';
  }
  else {
    radioString = 'Pfunctionradio1';
  }
  var selected = document.getElementById('Pselected');
  var selectedProgram = 0;
  if(selected.options[0] != null){
    selectedProgram = selected.options[0].value;
  }
  return "Mode=" + mode + "&ProgramSearch=" + document.getElementById('Pfunctioninput').value + "&SelectedProgram=" + selectedProgram + "&SearchRadio=" + radioString;
 
}

function updatepagePP(str) {
  if(str.length > 0){
    if(!isFireFox){
      document.getElementById("ppList").innerHTML = '<select class="longer" id="Pfunctionselect" name="Pfunctionselect" size="12"<% sendb(disabledattribute) %>>' + str + '</select>';
    }
    else{
      document.getElementById("Pfunctionselect").innerHTML = str;
    }
    document.getElementById("searchLoadDivPp").innerHTML = '&nbsp;';
    if (document.getElementById("Pfunctionselect").options.length > 0) {
      document.getElementById("Pfunctionselect").options[0].selected = true;
    }
  }
  else if(str.length == 0){
    if(!isFireFox){
      document.getElementById("ppList").innerHTML = '<select class="longer" id="Pfunctionselect" name="Pfunctionselect" size="12"<% sendb(disabledattribute) %>>&nbsp;</select>';
    }
    else{
      document.getElementById("Pfunctionselect").innerHTML = '&nbsp;';
    }
    document.getElementById("searchLoadDivPp").innerHTML = '&nbsp;';
  }
}
...
{
    xmlhttpPostPP('OfferFeeds.aspx','PointsProgramsCM');
...
function PupdateButtons(){
  ...
  if (elemDisallowEditPpOpt != null && elemDisallowEditPpOpt.value == '1') {
      ...
      if(document.getElementById("btncreatepoints")!=undefined && document.getElementById("btncreatepoints")!=null){
        document.getElementById("btncreatepoints").disabled=true;
        }
  } else {
    if (selectObj.length == 0) {
     ...
     if(document.getElementById("btncreatepoints")!=undefined && document.getElementById("btncreatepoints")!=null){
        document.getElementById("btncreatepoints").disabled=false;
        }
  }
}
...
function getproductquery(mode) {
  ...
  return "Mode=" + mode + "&ProductSearch=" + document.getElementById('functioninput').value + "&OfferID=" + document.getElementById('OfferID').value + "&SelectedGroup=" + selectedGroup + "&ExcludedGroup=" + excludedGroup + "&SearchRadio=" + radioString +  "&CallingPage=rewPoint";
...
function updatepage(str) {
  if(str.length > 0){
    if(!isFireFox){
      document.getElementById("pgList").innerHTML = '<select class="longer" id="functionselect" name="functionselect" size="20"<% sendb(disabledattribute) %>>' + str + '</select>';
	  $('#functionselect').css('width','250px');
    }
    else{
...
{
var returnval=true;
$("input[@name^=tier]").each(function(index){
  if(returnval==false)
    return false;
  if(!(this).disabled)
    {
        if(!IsNumeric((this).value, true,true))
      {

      
        alert('<%=Copient.PhraseLib.Lookup("error.requires_valid_number", LanguageID)%>');
        this.focus();
        this.select();
       returnval=false;
       return false;
...
if(!returnval)
  ...
  $("input[@name=Xbox]").each(function(){
  if(returnval==false)
      return false;
  if(!(this).disabled)
    {
        if(!IsNumeric((this).value, false,false))
      {
       alert('<%=Copient.PhraseLib.Lookup("error.requires_valid_positive_integer", LanguageID)%>');
        this.focus();
        this.select();
       returnval=false;
       return (false);
...
<%
    ...
    sTenderPointsId = MyCommon.NZ(MyCommon.Fetch_CM_SystemOption(37), "")
    If sTenderPointsId = "" Then
      MyCommon.QueryStr = "select ProgramID,ProgramName from PointsPrograms with (NoLock)" & _
                          " where deleted=0 and ProgramID is not null" & _
                          " order by ProgramName"
    Else
      MyCommon.QueryStr = "select ProgramID,ProgramName from PointsPrograms with (NoLock)" & _
                          " where deleted=0 and ProgramID is not null and ProgramID not in (" & sTenderPointsId & ")" & _
                          " order by ProgramName"
    End If
...
<%
    MyCommon.QueryStr = "select 0 as LimitID, 'None' as Name, RewardLimitTypeID as LimitTypeID, RewardLimit as LimitValue, RewardDistPeriod as LimitPeriod " & _
                        ...
                        "from CM_AdvancedLimits with (NoLock) where Deleted=0 and LimitTypeID=5 order By LimitID;"
    rst2 = MyCommon.LRT_Select
...
function handlePointsCreateClick(createbtn)
    {
        var alertMessage="";
        if(document.getElementById(createbtn)!= undefined && document.getElementById(createbtn) != null)
        {
            var searchText= document.getElementById('Pfunctioninput').value;
            if(searchText != null && searchText!="")
            {
                xmlhttpPost_CreatePointProgramFromOffer('OfferFeeds.aspx', 'CreateGroupOrProgramFromOffer');
            }
            else
            {
                alertMessage='<% Sendb(Copient.PhraseLib.Lookup("term.enter", LanguageID))%>' +' ' + '<% Sendb(Copient.PhraseLib.Lookup("term.valid", LanguageID).ToLower())%>'+' ' + '<% Sendb(Copient.PhraseLib.Lookup("term.pointsprogram", LanguageID).ToLower())%>'+' ' + '<% Sendb(Copient.PhraseLib.Lookup("term.name", LanguageID).ToLower())%>';
                alert(alertMessage);
                document.getElementById('Pfunctioninput').focus();
                return false;
            }
        }
        return true;
 ...
 function xmlhttpPost_CreatePointProgramFromOffer(strURL,mode) {
      var xmlHttpReq = false;
      var self = this;
      document.getElementById("searchLoadDivPp").innerHTML = '<% Sendb(Copient.PhraseLib.Lookup("message.loading", LanguageID)) %>';
      if (window.XMLHttpRequest) {
      ...
      var qryStr = getpointscreatequery(mode);
      self.xmlHttpReq.open('POST', strURL + "?" + qryStr, true);
      self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      self.xmlHttpReq.onreadystatechange = function() {
        if (self.xmlHttpReq !=null && self.xmlHttpReq.readyState == 4) {
          updatepage_createpointsprogramfromoffer(self.xmlHttpReq.responseText);
        }
      }

      self.xmlHttpReq.send(qryStr);
    }
    function getpointscreatequery(mode)
    {
        return "Mode=" + mode + "&CreateType=Points&Name=" + document.getElementById('Pfunctioninput').value
    }
    function updatepage_createpointsprogramfromoffer(str) 
    {
       if(str.length > 0)
      {
          var status ="";
          ...
          if(responseArr.length >0)
          {
            ...
            if(status=="Ok")
            {
                var resultArr=responseArr[1].split('|');
                 if(document.getElementById('Pselected').options[0] !=undefined && document.getElementById('Pselected').options[0]!=null){
                    document.getElementById('Pselected').options[0].selected = 'selected';
                }
                PhandleSelectClick('Pdeselect1');
                addNewPointsProgramtoSelectbox(resultArr[0],resultArr[1]);
                document.getElementById('Pfunctionselect').value=resultArr[1];
                PhandleSelectClick('Pselect1');
            }
            else if(status =="Fail")
            {
                var resultArr=responseArr[1].split('|');
                ...
                if(document.getElementById('Pselected').options[0] !=undefined && document.getElementById('Pselected').options[0]!=null)
                    selectedGroupValue= document.getElementById('Pselected').options[0].value
                ...
                {
                    alert('<% Sendb(Copient.PhraseLib.Lookup("term.pointsprogram", LanguageID))%>' + ": '"+ resultArr[0] + "' " + '<% Sendb(Copient.PhraseLib.Lookup("term.is", LanguageID).ToLower())%>'+'  '+'<% Sendb(Copient.PhraseLib.Lookup("term.already", LanguageID).ToLower())%>'+'  '+'<% Sendb(Copient.PhraseLib.Lookup("term.selected", LanguageID).ToLower())%>');
                }
            }
   ...
   function addNewPointsProgramtoSelectbox(text,val)
   {
        var sel = document.getElementById('Pfunctionselect');
...
<%
  ...
  For Each row In rst.Rows
    ...
    RewardLimitTypeID = MyCommon.NZ(row.Item("RewardLimitTypeID"), 5)
  ...
  MyCommon.QueryStr = "select RewardPointsID,ProgramID,ChargeBackDeptID,DiscountableItemsOnly,PrintLineText, WebText from RewardPoints where RewardPointsID=" & LinkID
  rst = MyCommon.LRT_Select()
  For Each row In rst.Rows
  ...
  ElseIf (Request.QueryString("save") <> "" Or Request.QueryString("clicksent") = "true" Or _
    ...
    If Not (bUseTemplateLocks And bDisallowEditAdv) Then
      ...
      If (Request.QueryString("disc-items-only") = "on") Then
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set DiscountableItemsOnly=1 where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
        DiscountableItemsOnly = True
      Else
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set DiscountableItemsOnly=0 where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
    ...
    If Not (bUseTemplateLocks And bDisallowEditPp) Then
      If (Request.QueryString("ProgramID") <> "") Then
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set ProgramID=" & Request.QueryString("ProgramID") & " where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
      Else
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set ProgramID=Null where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
    ...
    If Not (bUseTemplateLocks And bDisallowEditDept) Then
      If (Request.QueryString("dept") <> "") Then
        ...
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set ChargeBackDeptID=" & ChargeBackDeptID & " where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
      End If
    End If
    ...
    If Not (bUseTemplateLocks And bDisallowEditMsg) Then
      If (GetCgiValue("PrintLineText") <> "") Then
        ...
        MyCommon.QueryStr = "UPDATE RewardPoints with (RowLock) SET PrintLineText=@PrintLineText WHERE RewardPointsID=@RewardPointsID"
        MyCommon.DBParameters.Add("@PrintLineText", SqlDbType.NVarChar, 1000).Value = PrintLineText
        MyCommon.DBParameters.Add("@RewardPointsID", SqlDbType.BigInt).Value = LinkID
        MyCommon.ExecuteNonQuery(Copient.DataBases.LogixRT)
      Else
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set PrintLineText=null where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
      End If
      If (GetCgiValue("WebText") <> "") Then
        WebText = GetCgiValue("WebText")
        MyCommon.QueryStr = "UPDATE RewardPoints with (RowLock) SET WebText=@WebText WHERE RewardPointsID=@RewardPointsID"
        MyCommon.DBParameters.Add("@WebText", SqlDbType.NVarChar, 1000).Value = WebText
        MyCommon.DBParameters.Add("@RewardPointsID", SqlDbType.BigInt).Value = LinkID
        MyCommon.ExecuteNonQuery(Copient.DataBases.LogixRT)
      Else
        MyCommon.QueryStr = "update RewardPoints with (RowLock) set WebText=null where RewardPointsID=" & LinkID
        MyCommon.LRT_Execute()
      End If
    End If
    ...
    MyCommon.Activity_Log(3, OfferID, AdminUserID, Copient.PhraseLib.Lookup("history.rew-point", LanguageID))
...
<form action="offer-rew-point.aspx" id="mainform" name="mainform" onsubmit="return saveForm();">
  <div id="intro">
 ...
 <%Send("<div id=""infobar1"" >")
    ...
    <div id="columnfull">
      ...
      <%
        Send_ProductConditionSelectorAdv(Logix, TransactionLevelSelected, bUseTemplateLocks, Disallow_Edit, SelectedItem, ExcludedItem, RewardID, Copient.CommonInc.InstalledEngines.CM, IsTemplate, bDisallowEditPg, ByExistingPGSelector)
      ...
      <div class="box" id="message">
        ...
        <%
          ...
          End If
        %>
        (<% Send(Copient.PhraseLib.Lookup("offer-rew.silentpoints", LanguageID))%>)
        <hr class="hidden" />
        <br />
        <% If MyCommon.Fetch_CM_SystemOption(23) Then%>
        <div id="divWebText">
          <br />
          <label for="WebText" style="position: relative;"><% Sendb(Copient.PhraseLib.Lookup("term.webmessage", LanguageID))%>:</label>
          ...
          For Each row In rst.Rows
            ...
            If (row.Item("UseSpecialPricing") <> True) Then
              If (row.Item("Tiered") = False) Then
                Sendb("<input type=""text"" onkeypress=""return submitenter(event);"" class=""shorter"" id=""tier0"" maxlength=9 name=""tier0"" style=""width:35px;float:left;"" maxlength=""100"" value=""" & Int(MyCommon.NZ(row.Item("RewardAmount"), 0)) & """" & sDisabled & " />")
              Else
      ...
      <div class="box" id="limits">
        ...
        <%
          MyCommon.QueryStr = "Select LimitId, Name from CM_AdvancedLimits with (NoLock) where Deleted=0 and LimitTypeID=5 order By Name;"
          rst = MyCommon.LRT_Select
          If rst.Rows.Count > 0 Then
        %>
        ...
        <% Sendb(Copient.PhraseLib.Lookup("term.point(s)", LanguageID))%>
          ...
          If rst.Rows.Count > 0 Then
            ...
            For Each row In rst.Rows
              If (row.Item("UseSpecialPricing") = True) Then
                ...
                End If
                Send("    <td><input class=""shorter"" id=""tier" & q & """    maxlength=9 name=""tier" & q & """ type=""text"" value=""" & Int(MyCommon.NZ(row.Item("RewardAmount"), 0)) & """" & sDisabled & " />")
    ...
    <div id="column2">
      <div class="box" id="programs">
        ...
        <input class="medium" onkeyup="javascript:xmlPostTimerPP('OfferFeeds.aspx','PointsProgramsCM');" id="Pfunctioninput" name="Pfunctioninput" type="text" maxlength="100" value="" <% sendb(disabledattribute) %> />
        <% If (bCreateGroupOrProgramFromOffer AndAlso Logix.UserRoles.CreatePointsPrograms) Then%>
            <input class="regular" name="btncreatepoints" id="btncreatepoints" type="button" value="<% Sendb(Copient.PhraseLib.Lookup("term.create", LanguageID))%>" onclick="javascript:handlePointsCreateClick('btncreatepoints');" <% sendb(disabledattribute) %>/>
        <% End If%>
        <br />
        <div id="searchLoadDivPp" style="display: block;">&nbsp;</div>
        <div id="ppList">
        <select class="longer" id="Pfunctionselect" name="Pfunctionselect" size="10" <% sendb(disabledattribute) %>>
          <%
              'Retireieve all selected point programs
              Dim selectPoints As StringBuilder = New StringBuilder()
              Dim SelectedPoints As String = ""
              MyCommon.QueryStr = "select PP.ProgramID,PP.ProgramName from PointsPrograms as PP with (NoLock) " & _
                             "inner join RewardPoints as RP with (NoLock) on RP.ProgramID=PP.ProgramID " & _
                             "inner join OfferRewards as OFR with (NoLock) on OFR.LinkID=RP.RewardPointsID " & _
                             "where RewardID=" & RewardID & " and PP.Deleted=0 and OFR.Deleted=0;"
              rst2 = MyCommon.LRT_Select
              
              For Each row2 In rst2.Rows
                  SelectedPoints &= row2.Item("ProgramID") & ","
                  selectPoints.Append("<option value=""" & row2.Item("ProgramID") & """>" & row2.Item("ProgramName") & "</option>")
              Next
              
            sTenderPointsId = MyCommon.NZ(MyCommon.Fetch_CM_SystemOption(37), "")
              'Retreieve all point programs except selected point progrma
              MyCommon.QueryStr = "Select " & topString & "  pp.ProgramID, pp.ProgramName from PointsPrograms pp with (NoLock) where "
              MyCommon.QueryStr &= "  pp.ProgramID is not null and pp.Deleted = 0  "
              If Not String.IsNullOrEmpty(sTenderPointsId) Then
                  If (Not String.IsNullOrEmpty(SelectedPoints)) Then
                      MyCommon.QueryStr &= " and NOT EXISTS (SELECT e.ProgramID FROM PointsPrograms e WHERE e.ProgramID=pp.ProgramID and  e.ProgramID IN( " & SelectedPoints & sTenderPointsId & ")) "
                  Else
            ...
            Else
                  If (Not String.IsNullOrEmpty(SelectedPoints)) Then
                      MyCommon.QueryStr &= " and NOT EXISTS (SELECT e.ProgramID FROM PointsPrograms e WHERE e.ProgramID=pp.ProgramID and  e.ProgramID IN( " & SelectedPoints.Trim(",") & ")) "
                  End If
            End If
