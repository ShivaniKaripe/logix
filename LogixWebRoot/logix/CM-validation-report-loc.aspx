...
<%
  ' *****************************************************************************
  ' * FILENAME: CM-validation-report-loc.aspx 
  ' *~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Response.Expires = 0
  MyCommon.AppName = "CM-validation-report-loc.aspx"
  ...
  Select Case type
    Case "pg"
      MyCommon.QueryStr = "pa_CM_ValidationReport_LocProdGroups"
      MyCommon.Open_LRTsp()
      MyCommon.LRTsp.Parameters.Add("@LocationID", SqlDbType.Int).Value = id
      ...
      rows = rst.Select("Status=" & level, "Name")
      sTitle = Copient.PhraseLib.Lookup("term.productgroups", LanguageID)
    Case "cg"
      MyCommon.QueryStr = "pa_CM_ValidationReport_LocCustGroups"
      MyCommon.Open_LRTsp()
      MyCommon.LRTsp.Parameters.Add("@LocationID", SqlDbType.Int).Value = id
      MyCommon.LRTsp.Parameters.Add("@GraceHours", SqlDbType.Int).Value = iGraceHours
      MyCommon.LRTsp.Parameters.Add("@GraceHoursWarn", SqlDbType.Int).Value = iGraceHoursWarn
      ...
      MyCommon.Close_LRTsp()
      rows = rst.Select("Status=" & level, "Name")
      sTitle = Copient.PhraseLib.Lookup("term.customergroups", LanguageID)
    Case "in"
      MyCommon.QueryStr = "pa_CM_ValidationReport_LocOffers"
      MyCommon.Open_LRTsp()
      MyCommon.LRTsp.Parameters.Add("@LocationID", SqlDbType.Int).Value = id
      MyCommon.LRTsp.Parameters.Add("@GraceHours", SqlDbType.Int).Value = iGraceHours
      MyCommon.LRTsp.Parameters.Add("@GraceHoursWarn", SqlDbType.Int).Value = iGraceHoursWarn
      ...
      MyCommon.Close_LRTsp()
      rows = rst.Select("Status=" & level, "Name")
      sTitle = Copient.PhraseLib.Lookup("term.offers", LanguageID)
  End Select
  
  IntroText = Copient.PhraseLib.Lookup("term.store", LanguageID) & " #" & id & ": "
  If (level = 0) Then
    IntroText += " " & Copient.PhraseLib.Lookup("term.valid", LanguageID) & " " & sTitle
  ElseIf (level = 1) Then
    IntroText += " " & Copient.PhraseLib.Lookup("term.waiting", LanguageID) & " " & sTitle
  ElseIf (level = 2) Then
    IntroText += " " & Copient.PhraseLib.Lookup("term.watches", LanguageID) & " " & sTitle
  ElseIf (level = 3) Then
    IntroText += " " & Copient.PhraseLib.Lookup("term.warnings", LanguageID) & " " & sTitle
  End If
...
<%
  If (Not rows is Nothing AndAlso rows.length > 0)
    ...
    For Each row In rows
      sName = MyCommon.NZ(row.Item("Id").ToString(), "*") & " - " & MyCommon.NZ(row.Item("Name").ToString(), "")
      If (sName.Length < 37) Then
        sName = sName.PadRight(37, " ")
      Else
        sName = sName.Substring(0, 33) & "... "
      End If
      OptionText =  row.Item("StatusMessage")
      If (OptionText = "") Then
        OptionText = sName & GetReasonText(row.Item("ReturnCode"))
      Else
        OptionText = sName & OptionText
      End If
...
<%
  ...
  If (Logix.UserRoles.AccessStoreHealth = False) Then
    Send_Denied(2, "perm.admin-store-health")
    GoTo done
  End If
%>
              ...
              If (Not rows Is Nothing) Then
                For Each row In rows
                  ...
                  Else
                    sName = sName.Substring(0, 33) & "... "
                  End If
                  OptionText = sName.Replace(" ", "&nbsp;")
                  Sendb("<option value=""" & MyCommon.NZ(row.Item("ID"), 0) & """>" & OptionText)
                  OptionText = MyCommon.NZ(row.Item("StatusMessage"), "")
                  If OptionText = "" Then
