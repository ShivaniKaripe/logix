...
<%
  ...
  If (Request.QueryString("Mode") = "MarkupTags") Then
    ...
    GetMarkupTags(EngineID, OfferID, Phase, PrinterTypeID, MyCommon)
  Else
...
<script runat="server">
  ...
  Function isOfferTargeted(ByRef MyCommon As Object, ByVal EngineID As Integer, ByVal OfferID As Long) As Boolean
  ...
  Sub GetMarkupTags(ByVal EngineID As Integer, ByVal OfferID As Long, ByVal Phase As Integer, ByVal PrinterTypeID As Integer, ByRef MyCommon As Copient.CommonInc)
    ...
    Try
      ...
      TargetedOffer = isOfferTargeted(MyCommon, EngineID, OfferID)
      ...
      If (rst.Rows.Count > 0) Then
        For Each row In rst.Rows
          ...
          ElseIf (CleanID = "SVVAL") Or (CleanID = "SVBAL") Or (CleanID = "SVBALEXP") Or (CleanID = "SVVALEXP") Or (CleanID = "SVLIMIT") Or (CleanID = "SVREDEEM") Or (CleanID = "SVVAL") Or (CleanID = "SVVALNET") Or (CleanID = "SVVALINITIAL") Or (CleanID = "SVVALEARNED") Or (CleanID = "SVVALREDEEMED") Then
          ...
          ElseIf (CleanID = "SVEXP_EOM") Then
			If ( MyCommon.Fetch_CPE_SystemOption(149)="1" And iRewardTypeId <> 3  ) Then
 			  Sendb("<input type=""button"" id=""ed_" & (StrConv(CleanID, VbStrConv.Lowercase)) & """ class=""ed_button""" & DisabledAttribute & " title=""" & Copient.PhraseLib.Lookup(MyCommon.NZ(row.Item("PhraseID"), 0), LanguageID) & """ onclick=""javascript:showDialogSpan(true, 9, this.value);"" value=""" & (StrConv(MyCommon.NZ(row.Item("ButtonText"), ""), VbStrConv.ProperCase)) & """ />")
			Else
			  Sendb("<input type=""button"" id=""ed_" & (StrConv(CleanID, VbStrConv.Lowercase)) & """ class=""ed_button""" & DisabledAttribute & " title=""" & Copient.PhraseLib.Lookup(MyCommon.NZ(row.Item("PhraseID"), 0), LanguageID) & """ onclick=""javascript:showDialogSpan(true, 3, this.value);"" value=""" & (StrConv(MyCommon.NZ(row.Item("ButtonText"), ""), VbStrConv.ProperCase)) & """ />")
		    End If
          ...
          ElseIf (CleanID = "DSCORECARD") Then
            ...
            End If
          ElseIf (CleanID = "LSCORECARD") Then
            If ScorecardWorthy Then
              Sendb("<input type=""button"" id=""ed_" & (StrConv(CleanID, VbStrConv.Lowercase)) & """ class=""ed_button""" & DisabledAttribute & " title=""" & Copient.PhraseLib.Lookup(MyCommon.NZ(row.Item("PhraseID"), 0), LanguageID) & """ onclick=""javascript:showDialogSpan(true, 8, this.value);"" value=""" & (StrConv(MyCommon.NZ(row.Item("ButtonText"), ""), VbStrConv.ProperCase)) & """ />")
            End If
