...
<WebService(Namespace:="http://www.copienttech.com/PointsUpdateConnector/")> _
...
Public Class Service
  ...
  Private LogFile As String = "PointsUpdateWSLog"
  Private Const CONNECTOR_ID As Integer = 48
  ...
  <WebMethod()> _
  Public Function AdjustPoints(ByVal GUID As String, ByVal UpdateStr As String) As XmlDocument
    
    Dim MethodName As String = "AdjustPoints"
    ...
    Dim xsdName As String = "PointsUpdate.xsd"
    Dim xsdError As String = ""
    Dim RetCode As StatusCodes = StatusCodes.SUCCESS
    Dim PointsAdj As Copient.Points
    ...
    If Not (IsValidGUID(GUID, MethodName, Common)) Then
      Generate_Status_XML(Writer, MethodName, StatusCodes.INVALID_GUID, "GUID '" & GUID & "' is not valid for the Points Update web service.", False)
    Else
      ...
      If ConnectorInc.ConvertStringToXML(UpdateStr, UpdateXML) Then
        ...
        If ValidXMLDoc Then
          PointsAdj = New Copient.Points(Common)

          'process the XML that was passed in
          StatusDesc = PointsAdj.Process_PointsUpdateXML(UpdateStr, RetCode, 3)
          Generate_Status_XML(Writer, MethodName, RetCode, StatusDesc, (RetCode = StatusCodes.SUCCESS))
          Common.Write_Log(LogFile, StatusDesc, True)
        Else
...
ErrorHandler:
    ...
    Common.Write_Log(LogFile, "An error occurred while processing - please see the error log!", True)
...
Finish:
  ...
  Private Sub Init_App()
    Common.Populate_AppInfo(500)
    
    LogFile = Common.Assign_LogFile_Name(LogFile)
  End Sub
  ...
  Private Sub Init_ResponseXML(ByRef sw As StringWriter, ByRef Writer As XmlWriter)
    ...
    Writer.WriteStartElement("PointsUpdate")
  End Sub
  ...
  Private Sub Close_ResponseXML(ByRef Writer As XmlWriter)
    Writer.WriteEndElement() 'PointsUpdate
 ...
 '-------------------------------------------------------------------------------------------------------------------------------------
  ...
  Private Function IsValidGUID(ByVal GUID As String, ByVal MethodName As String, ByRef Common As Copient.CommonInc) As Boolean
    ...
    Try
      ...
      Common.Write_Log(LogFile, MsgBuf.ToString, True)
    Catch ex As Exception
      ' ignore
    End Try
