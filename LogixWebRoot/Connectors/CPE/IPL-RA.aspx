...
<script runat="server">
  ...
  Function Construct_Table(ByVal TableName As String, ByVal Operation As Integer, ByVal DelimChar As Integer, ByVal LocalServerID As String, ByVal LocationID As String, ByVal DBName As String) As String
    ...
    If reader.HasRows Then
      If UCase(TableName) = "REWARDACCUMULATION" Then
        FieldList = "RewardAccumulationID" & Chr(DelimChar) & "RewardOptionID" & Chr(DelimChar) & "UserID" & Chr(DelimChar) & "QtyPurchased" & Chr(DelimChar) & "TotalPrice" & Chr(DelimChar) & "AccumulationDate" & Chr(DelimChar) & "OverThreshold" & Chr(DelimChar) & "LocalID" & Chr(DelimChar) & "ServerSerial" & Chr(DelimChar) & "WaitingACK"
      End If
      ...
      If UCase(TableName) = "REWARDACCUMULATION" And Operation = 5 Then
        LastTable = "RewardAccumulation"
        While reader.Read
          ...
          If ( buf.Length > BUFFERED_WRITE_SIZE )
            ...
            buf.Clear()
          End If
          LastPK = reader.Item("RewardAccumulationID")
          NumRecs = NumRecs + 1
        End While
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long)
    ...
    End If
    'RewardAccumulation
    Common.QueryStr = "select RA.RewardAccumulationID, convert(nvarchar, RA.RewardAccumulationID) + char(30) + convert(nvarchar, isnull(RA.RewardOptionID, 0)) + char(30) + convert(nvarchar, isnull(RA.CustomerPK, 0)) + char(30) + convert(nvarchar, isnull(QtyPurchased, 0)) + char(30) + convert(nvarchar, isnull(TotalPrice, 0)) + char(30) + convert(varchar, isnull(AccumulationDate, '1/1/1980'), 20) + char(30) + convert(nvarchar, isnull(OverThreshold, 0))  + char(30) + convert(nvarchar, isnull(LocalID, 0)) + char(30) + convert(nvarchar, isnull(ServerSerial, 0)) + char(30) + '2' as data " & _
                        "INTO #tmp_ra " & _
                        "from CPE_RewardAccumulation as RA with (NoLock, Index=IX_CPE_RewardAccumulation_CustomerPK) " & _
                        "Inner Join CustomerLocations as CL with (NoLock, Index=PK_CustomerLocations) on RA.CustomerPK=CL.CustomerPK " & _
                        "where CL.LocationID=" & LocationID & " and RA.Deleted=0"
    Common.LXS_Execute()
    
    Common.QueryStr = "SELECT " & MaxBatchStr & " * FROM #tmp_ra AS RA "

    If StartPK > 0 Then
      Common.QueryStr = Common.QueryStr & "WHERE RA.RewardAccumulationID>" & StartPK & " "
    End If
    Common.QueryStr = Common.QueryStr & "ORDER BY RA.RewardAccumulationID"

    OutStr = OutStr & Construct_Table("RewardAccumulation", 5, DelimChar, LocalServerID, LocationID, "LXS")
...
<%
  ...
  ElseIf Not (Connector.IsValidLocationForConnectorEngine(Common, LocationID, 2)) Then
    'the location calling IPL-RA is not associated with the CPE promoengine
    ...
    ElseIf (Mode = "IPL") Then
      ...
      Else
        ...
        If StartTable = "" Then  'only perform these actions the first time IPL-PG is called
          ...
          Common.QueryStr = "dbo.pc_CPE_IPLPurge_RA_Output"
          Common.Open_LXSsp()
