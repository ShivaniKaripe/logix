...
<script runat="server">
  ...
  Function Construct_Table(ByVal TableName As String, ByVal Operation As Integer, ByVal DelimChar As Integer, ByVal LocalServerID As String, ByVal LocationID As String, ByVal DBName As String) As String
    ...
    If reader.HasRows Then
      If UCase(TableName) = "STOREDFRANKING" Then
        FieldList = "userid" & Chr(DelimChar) & "roid" & Chr(DelimChar) & "output" & Chr(DelimChar) & "create_date" & Chr(DelimChar) & "status" & Chr(DelimChar) & "priority" & Chr(DelimChar) & "deliverabletype"
      End If
      ...
      If UCase(TableName) = "STOREDFRANKING" And (Operation = 5) Then
        While reader.Read
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long)
    ...
    Common.QueryStr = "select " & _
                      " convert(nvarchar, SF.customerpk) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.rewardid) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.franking_text) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.create_date) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.status) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.priority) + char(" & DelimChar & ") +" & _
                      " convert(nvarchar, SF.deliverabletype) as data " & _
                      "from StoredFranking as SF with (NoLock) " & _
                      "inner join CustomerLocations as CL with (NoLock) on SF.customerpk=CL.CustomerPK " & _
                      "where Status=1 and CL.LocationID=" & LocationID & ";"
    OutStr = OutStr & Construct_Table("StoredFranking", 5, DelimChar, LocalServerID, LocationID, "LXS")
...
<%
  ...
  ElseIf Not (Connector.IsValidLocationForConnectorEngine(Common, LocationID, 2)) Then
    'the location calling IPL-SF is not associated with the CPE promoengine
  ...
  Else
    ...
    ElseIf (Mode = "IPL") Then
      ...
      Else
        ...
        Common.QueryStr = "dbo.pc_CPE_IPLPurge_SF_Output"
        Common.Open_LXSsp()
