...
<script runat="server">
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long)
    ...
    'RewardAccumulation
    Common.QueryStr = "select  " & MaxBatchStr & " RA.RewardAccumulationID, convert(nvarchar, RA.RewardAccumulationID) + char(30) + convert(nvarchar, isnull(RA.RewardOptionID, 0)) + char(30) + convert(nvarchar, isnull(RA.CustomerPK, 0)) + char(30) + convert(nvarchar, isnull(QtyPurchased, 0)) + char(30) + convert(nvarchar, isnull(TotalPrice, 0)) + char(30) + convert(varchar, isnull(AccumulationDate, '1/1/1980'), 20) + char(30) + convert(nvarchar, isnull(OverThreshold, 0))  + char(30) + convert(nvarchar, isnull(LocalID, 0)) + char(30) + convert(nvarchar, isnull(ServerSerial, 0)) + char(30) + '2' as data  " & _
               "from CPE_RewardAccumulation as RA with (NoLock) Inner Join CustomerLocations as CL with (NoLock) on RA.CustomerPK=CL.CustomerPK " & _
               "where CL.LocationID=" & LocationID & " and RA.Deleted=0 "
    If StartPK > 0 Then
      Common.QueryStr = Common.QueryStr & "and RA.RewardAccumulationID>" & StartPK & " "
    End If
  ...
  Else
    ...
    ElseIf (Mode = "IPL") Then
      ...
      Else
        ...
        SD(ApplicationName & "," & Connector.CSMajorVersion & "," & Connector.CSMinorVersion & "," & Connector.CSBuild & "," & Connector.CSBuildRevision)
        SD("LocationID=" & LocationID)
