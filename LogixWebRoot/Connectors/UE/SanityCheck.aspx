...
<script runat="server">
  ...
  Sub CheckTables_Init()
    ...
    'If UE System option 80 is enabled then do not find the expired offers.
    If OperateAtEnterprise Then
      ...
      Common.QueryStr = "Insert into #ActiveIncentives (IncentiveID) " & _
                        ...
                        "where (OLU.LocationID=" & LocationID & ") " & IIf(Common.Fetch_UE_SystemOption(80) = "1", " and ExpireLocked=0 ", "") & " " & _
                        " union " & _
                        ...
                        "where (L.LocationID=" & LocationID & ") " & IIf(Common.Fetch_UE_SystemOption(80) = "1", " and ExpireLocked=0 ", "") & " " & _
    ...
    CheckTables(tableNum, Query) = "select Distinct CG.CustomerGroupID from CustomerGroups as CG with (NoLock) Inner Join CustomerGroupLocUpdate as CGLU with (NoLock) on CG.CustomerGroupID=CGLU.CustomerGroupID and CG.Deleted=0 and CGLU.LocationID=" & LocationID & " and CG.CustomerGroupID <> 1 " & _
                                   "union " & _
                                   "select distinct CustomerGroupID from CustomerGroups with (NoLock) where AnyCardholder=1 or NewCardholders=1 or AnyCAMCardholder=1 " & _
  ...
  Sub GroupTables_Init()
    ...
    '     the GQueryStart index, the actually group id, and the GQueryEnd index
...
<%
  ...
  LastHeard = "1/1/1980"
  LocalServerID = Common.Extract_Val(Request.QueryString("serial"))
  LogFile = "UE-SanityCheckLog-" & Format(LocalServerID, "00000") & "." & Common.Leading_Zero_Fill(Year(Today), 4) & Common.Leading_Zero_Fill(Month(Today), 2) & Common.Leading_Zero_Fill(Microsoft.VisualBasic.DateAndTime.Day(Today), 2) & ".txt"
  LocalServerIP = Trim(Request.QueryString("IP"))
  MacAddress = Trim(Request.QueryString("mac"))
  If MacAddress = "" Or MacAddress = "0" Then
    MacAddress = "0"
