    ...
    If dst.Rows.Count > 0 Then
      ...
      If UCase(TableName) = "USERS" And Operation = 1 Then
        For Each row In dst.Rows
          SD(Common.NZ(row.Item("UserID"), 0) & DelimChar & Common.NZ(row.Item("ClientUserID1"), " ") & DelimChar & Common.NZ(row.Item("HHPrimaryID"), 0) & DelimChar & Common.NZ(row.Item("HHrec"), 0) & DelimChar & Common.NZ(row.Item("CustomerTypeID"), 0) & DelimChar & row.Item("CustomerStatusID") & DelimChar & row.Item("AlternateID") & DelimChar & row.Item("Verifier") & DelimChar & Parse_Bit(row.Item("Employee")) & DelimChar & row.Item("AltIDOptOut") & DelimChar & row.Item("FirstName") & DelimChar & row.Item("LastName") & DelimChar & row.Item("EmployeeID") & DelimChar & row.Item("AirmileMemberID") & DelimChar & row.Item("Prefix") & DelimChar & row.Item("Suffix"))
      ...
      ElseIf UCase(TableName) = "CARDIDS" And Operation = 1 Then
        For Each row In dst.Rows
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long, ByVal LastHeard As String)
    ...
    If Not (MustIPL) Then
      ...
      If Not (TotalTime > 240) Then
        ...
        dst = Common.LXSsp_select
        Common.Close_LXSsp()
        Construct_Table("CardIDs", 1, DelimChar, LocalServerID, LocationID, dst)
        dst = Nothing
      Else
      ...
      If Not (TotalTime > 240) Then
        ...
        dst = Common.LXSsp_select
        Common.Close_LXSsp()
        ColumnNamesOverride = "ProgramID" & DelimChar & "UserID" & DelimChar & "AdjAmount"
        Construct_Table("PointsAdj", 5, DelimChar, LocalServerID, LocationID, dst, ColumnNamesOverride)
        dst = Nothing
      Else
      ...
      If Common.IsIntegrationInstalled(Copient.CommonInc.Integrations.PREFERENCE_MANAGER) And Common.Fetch_UE_SystemOption(134) = "1" Then
      ...
      End If  'Common.IsIntegrationInstalled(Copient.CommonInc.Integrations.PREFERENCE_MANAGER) And Common.Fetch_UE_SystemOption(134) = "1"
  ...
  Else
    Select Case Mode
      ...
      Case "FETCH"
        ...
        Common.Write_Log(LogFile, "Total uncompressed size = " & Format(UncompressedSize, "###,###,###,###,##0") & " bytes")
        TotalTime = DateAndTime.Timer - StartTime
