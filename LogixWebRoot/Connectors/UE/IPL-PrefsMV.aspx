...
<script runat="server">
  ...
  Function Construct_Table(ByVal TableName As String, ByVal Operation As Integer, ByVal DelimChar As Integer, ByVal LocalServerID As String, ByVal LocationID As String, ByVal DBName As String) As String
    ...
    If reader.HasRows Then
      If UCase(TableName) = "CUSTOMERPREFERENCESMV" Then
      ...
      If UCase(TableName) = "CUSTOMERPREFERENCESMV" And (Operation = 5) Then
        While reader.Read
          SD(reader.Item("Data"))
          NumRecs = NumRecs + 1
        End While
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long)
    ...
    Common.QueryStr = "select convert(nvarchar, P.CustomerPK) + char(" & DelimChar & ") + convert(nvarchar, P.PreferenceID) + char(" & DelimChar & ") + isnull(P.Value, '') as Data " & _
             "from CustomerPreferencesMV as P with (NoLock) Inner Join CustomerLocations as CL with (NoLock) on P.CustomerPK=CL.CustomerPK " & _
             "where CL.LocationID=" & LocationID & " and P.Deleted=0;"
    OutStr = OutStr & Construct_Table("CustomerPreferencesMV", 5, DelimChar, LocalServerID, LocationID, "LXS")
  ...
  Sub Process_IPL(ByVal IPLTypeID As Integer, ByVal LocalServerID As Long, ByVal LocationID As Long, ByVal IPLSessionID As String, ByVal HistoryStartTime As DateTime)
    ...
    Else
      ...
      Common.QueryStr = "dbo.pc_CPE_IPLPurge_PrefsMV_Output"
      Common.Open_LXSsp()
...
<%
  ...
  ElseIf Not (Connector.IsValidLocationForConnectorEngine(Common, LocationID, 9)) Then
    'the location calling IPL-PrefsMV is not associated with the CPE promoengine
  ...
  Else 'LocationID is valid
    ...
    Else 'Something is not enabled
      ...
      Else
        ...
        Common.Write_Log(LogFile, "Preference data distribution (UESystemOptionID 134) is disabled - NAK returned to caller")
        Process_NAK(LocalServerID, LocationID)
      End If
