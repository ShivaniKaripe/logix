...
<script runat="server">
  ...
  Dim bTranslateProductType As Boolean = IIf(Common.Fetch_SystemOption(220) = "1", True, False)
  'cloudsol-1002++  
  ...
  Private Sub SetUpDependency()
    Dim testBuilder As New WebRequestResolverBuilder
    testBuilder.Build()
    CurrentRequest.Resolver = testBuilder.GetResolver()
    CurrentRequest.Resolver.AppName = "IPL-PG"
  ...
  'CloudSol-1002
  Sub CheckThreshold(ByVal DeployingProductsCount As Int32)
    Dim DeployedProductsCount As Int32, Threshhold As Int32
    'Change the system option
    Threshhold = Convert.ToInt32(IIf(Common.Fetch_SystemOption(233) = "", 0, Common.Fetch_SystemOption(233)))

    'Find the deployed produts count
    Common.QueryStr = "dbo.pa_GetDeployedProductItemsCount"
    Common.Open_LRTsp()
    DeployedProductsCount = Convert.ToInt32(Common.LRTsp.ExecuteScalar())
    Common.Close_LRTsp()
    If (Threshhold <> 0 AndAlso DeployedProductsCount + DeployingProductsCount > Threshhold) Then
      Common.Error_Processor("Number of Products deployed exceeded the threshold configured!", "No of products deployed exceeded the threshold configured!", Common.AppName, "", Common.AppID)
    End If
  End Sub
    ...
    If reader.HasRows Then
      ...
      If UCase(TableName) = "PRODUCTGROUPS" And (Operation = 5) Then
        While reader.Read
          ...
          If StartTable = "" Then
            SD(reader.Item("ProductGroupID") & Chr(DelimChar) & Common.NZ(reader.Item("GroupName"), "Unknown") & Chr(DelimChar) & Parse_Bit(Common.NZ(reader.Item("AnyProduct"), 0)) & Chr(DelimChar) & Parse_Bit(Common.NZ(reader.Item("PointsNotApplyGroup"), 0)) & Chr(DelimChar) & Parse_Bit(Common.NZ(reader.Item("NonDiscountableGroup"), 0)) & Chr(DelimChar) & Common.NZ(reader.Item("UpdateLevel"), ""))
          End If
          NumRecs = NumRecs + 1
        End While
      ...
      ElseIf UCase(TableName) = "PRODGROUPITEMS" And (Operation = 5) Then
        ...
        End While
        ''cloudsol-1002++
        m_productService.CheckThreshold(NumRecs)
        'CheckThreshold(NumRecs)
  ...
  Sub Construct_Output(ByVal LocalServerID As Long, ByVal LocationID As Long)
    ...
    'ProductGroups
    Common.QueryStr = "select PG.ProductGroupID, PG.Name as GroupName, PG.AnyProduct, PG.PointsNotApplyGroup, PG.NonDiscountableGroup, PG.UpdateLevel " & _
               "from ProductGroups as PG with (NoLock) Inner Join UE_ProductGroupLocUpdate as PGLU with (NoLock) on PG.ProductGroupID=PGLU.ProductGroupID and PG.Deleted=0 " & _
               "Where (PGLU.LocationID=" & LocationID & " and PGLU.EngineID=9) " & _
               "union " & _
               ...
               "where PG.AnyProduct=1 or PG.PointsNotApplyGroup=1 or PG.NonDiscountableGroup=1 " & _
               "order by ProductGroupID;"
    ProductGroupIDStr = Construct_Table("ProductGroups", 5, DelimChar, LocationID, LocationID, "LRT")

    'If StartTable = "" Or UCase(StartTable) = "PRODGROUPITEMS" Then
    'If LastTable = "" Then
    'send the Entire List of products for each of the product groups we just sent
        
    If (bTranslateProductType) Then
      Common.QueryStr = " select distinct  " & MaxBatchStr & "  PGI.PKID, convert(nvarchar,PGI.ProductGroupID) + char(" & DelimChar & ") + " & _
                        " dbo.fn_GetProductAlternateCode(P.ExtProductID,P.ProductTypeID)  + char(" & DelimChar & ") + " & _
                        " CASE WHEN  P.ProductTypeID=3 THEN '6' WHEN  P.ProductTypeID=4 THEN '6' WHEN  P.ProductTypeID=5 THEN '6' ELSE convert(nvarchar,P.ProductTypeID)  " & _
                        " END as data " & _
                        " From ProdGroupItems as PGI with (NoLock) Inner Join Products as P with (NoLock) on PGI.ProductID=P.ProductID and PGI.Deleted=0  " &
                        " INNER JOIN (select PG.ProductGroupID " & _
                        " from ProductGroups as PG with (NoLock) Inner Join UE_ProductGroupLocUpdate as PGLU with (NoLock) on PG.ProductGroupID=PGLU.ProductGroupID and PG.Deleted=0 " & _
                        " Where (PGLU.LocationID=" & LocationID & " and PGLU.EngineID=9) " & _
                        " union " & _
                        " select PG.ProductGroupID " & _
                        " from ProductGroups as PG with (NoLock) " & _
                        " where PG.AnyProduct=1 or PG.PointsNotApplyGroup=1 or PG.NonDiscountableGroup=1) " & _
                        " AS PGS ON PGS.ProductGroupID = PGI.ProductGroupID "
      If StartPK > 0 Then
        Common.QueryStr = Common.QueryStr & "where PGI.PKID>" & StartPK & " "
      End If
      Common.QueryStr = Common.QueryStr & "order by PGI.PKID;"
    Else
      Common.QueryStr = " select distinct " & MaxBatchStr & " PGI.PKID, convert(nvarchar,PGI.ProductGroupID) + char(" & DelimChar & ") + P.ExtProductID + char(" & DelimChar & ") + convert(nvarchar,P.ProductTypeID)as data " & _
                        " From ProdGroupItems as PGI with (NoLock) Inner Join Products as P with (NoLock) on PGI.ProductID=P.ProductID and PGI.Deleted=0  " &
                        " INNER JOIN (select PG.ProductGroupID " & _
                        " from ProductGroups as PG with (NoLock) Inner Join UE_ProductGroupLocUpdate as PGLU with (NoLock) on PG.ProductGroupID=PGLU.ProductGroupID and PG.Deleted=0 " & _
                        " Where (PGLU.LocationID=" & LocationID & " and PGLU.EngineID=9) " & _
      ...
      'and PGI.ProductGroupID in " & ProductGroupIDStr & " "
      If StartPK > 0 Then
        Common.QueryStr = Common.QueryStr & "where PGI.PKID>" & StartPK & " "
      End If
      Common.QueryStr = Common.QueryStr & "order by PGI.PKID;"
    ...
    ElseIf (Mode = "IPL") Then
      ...
      Else
        ...
        If StartTable = "" Then  'only perform these actions the first time IPL-PG is called
          'Purge any ProductGroups that are no longer used in active offers from UE_ProductGroupLocUpdate for this location
          TotalTime = DateAndTime.Timer - StartTime
          Common.Write_Log(LogFile, "Removing expired UE_ProductGroupLocUpdate data - Total elapsed time=" & Int(TotalTime) & Format$(TotalTime - Fix(TotalTime), ".000") & "(sec)")
          If OperateAtEnterprise Then
            Common.QueryStr = "pa_Purge_Expired_UE_ProductGroupLocUpdate_Enterprise"
          Else
            Common.QueryStr = "pa_Purge_Expired_UE_ProductGroupLocUpdate"
          End If
          Common.Open_LRTsp()
          Common.LRTsp.Parameters.Add("@LocationID", SqlDbType.BigInt).Value = LocationID
          Common.LRTsp.ExecuteNonQuery()
          Common.Close_LRTsp()
          TotalTime = DateAndTime.Timer - StartTime
          Common.Write_Log(LogFile, "Finished removing expired UE_ProductGroupLocUpdate data - Total elapsed time=" & Int(TotalTime) & Format$(TotalTime - Fix(TotalTime), ".000") & "(sec)")
        End If
