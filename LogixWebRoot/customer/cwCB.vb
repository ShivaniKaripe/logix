...
Public Class cwCB
  ...
  Public CustLanguageID As Integer = 1
  ...
  Public Sub Send_Metas(ByVal Handheld As Boolean)
    Send("<meta name=""copyright"" content=""&copy; Copyright 2011, NCR"" />")
    Send("<meta name=""description"" content=""NCR Store customer-facing website"" />")
  ...
  Public Sub Send_Links(ByVal Handheld As Boolean)
    ...
    If (Handheld) Then
      Send("<link rel=""stylesheet"" href=""css/cw-handheld.css"" type=""text/css"" media=""handheld, screen"" />")
    Else
      Send("<link rel=""stylesheet"" href=""css/cw-screen.css"" type=""text/css"" media=""screen"" />")
    End If
  End Sub
  ...
  Public Sub Send_Logo(ByVal Handheld As Boolean)
    If (Handheld) Then
      ...
      Send("    <a href=""home.aspx""><img src=""images/logo-mini.jpg"" alt="""" title="""" /></a>")
      Send("  </td>")
      Send("</tr>")
    Else
      Send("<div id=""logos"">")
      Send("  <img src=""images/logo.png"" id=""logo"" alt="""" title="""" />")
      Send("</div>")
    End If
    Send("")
  End Sub
  ...
  Public Sub Send_Login(ByVal Handheld As Boolean)
    If (Handheld) Then
      ...
      Send("  <form action=""home.aspx"" method=""post"" id=""loginform"" name=""loginform"" target=""_top"">")
  ...
  Public Sub Send_Details(ByVal Handheld As Boolean)
    ...
    Send("  <form id=""editform"" method=""post"" action=""home.aspx"">")
        ...
        If (rowCount > 0) Then
          ...
          For Each row In rst.Rows
            MyCommon.QueryStr = "SELECT O.OfferID,O.ExtOfferID,O.IsTemplate,O.CMOADeployStatus,O.StatusFlag,O.OddsOfWinning, O.InstantWin, " & _
                                "O.Name,O.Description,O.ProdStartDate,O.ProdEndDate from CM_ST_Offers as O " & _
                                "LEFT JOIN CM_ST_OfferConditions as OC on OC.OfferID=O.OfferID " & _
                                ...
                                "from CPE_ST_Incentives I LEFT JOIN CPE_ST_RewardOptions RO on I.IncentiveID = RO.IncentiveID and RO.TouchResponse = 0 and RO.Deleted =0 " & _
                                "LEFT JOIN CPE_ST_IncentiveCustomerGroups  ICG on RO.RewardOptionID = ICG.RewardOptionID " & _
            ...
            For Each row2 In rst2.Rows
              ...
              'Get LinkID
              MyCommon.QueryStr = "SELECT LinkID FROM CM_ST_OfferConditions WHERE ConditionTypeID=1 AND Deleted=0 AND OfferID=" & row2.Item("OfferID")
              ...
              MyCommon.QueryStr = "SELECT 1 as EngineID, O.OfferID,LinkID,ProgramName,PP.ProgramID,PromoVarID " & _
                                   "FROM CM_ST_OfferRewards OFR with (NoLock) " & _
                                   "LEFT JOIN CM_ST_RewardPoints  RP with (NoLock) ON RP.RewardPointsID=OFR.LinkID " & _
                                   "LEFT JOIN PointsPrograms  PP with (NoLock) ON RP.ProgramID=PP.ProgramID " & _
                                   "LEFT JOIN CM_ST_Offers O with (NoLock) ON O.OfferID=OFR.OfferID " & _
                                   ...
                                   "LEFT JOIN CPE_ST_DeliverablePoints DP with (NoLock) on PP.ProgramID=DP.ProgramID " & _
                                   "INNER JOIN CPE_ST_Deliverables D with (NoLock) on D.DeliverableID=DP.DeliverableID " & _
                                   "INNER JOIN CPE_ST_RewardOptions RO with (NoLock) on RO.RewardOptionId = D.RewardOptionID " & _
      ...
      If (rst.Rows.Count > 0) Then
        ...
        For Each row In rst.Rows
          MyCommon.QueryStr = "SELECT distinct O.OfferID,O.ExtOfferID,O.IsTemplate,O.CMOADeployStatus,O.StatusFlag,O.OddsOfWinning, O.InstantWin, " & _
                              "O.Name,O.Description,O.ProdStartDate,O.ProdEndDate, LinkID from CM_ST_Offers as O " & _
                              "LEFT JOIN CM_ST_OfferConditions as OC on OC.OfferID=O.OfferID " & _
    ...
    If (Identifier <> "") Then
      ...
      If (rst.Rows.Count > 0) Then
        ...
        MyCommon.QueryStr = "select I.IncentiveID, I.IncentiveName, I.Description, I.StartDate, I.EndDate, ICG.CustomerGroupID, ICG.ExcludedUsers, D.OutputID as RewardGroup, I.AllowOptOut, RO.RewardOptionID " & _
                            "from CPE_ST_Incentives I inner join OfferIDs OID on I.IncentiveID = OID.OfferID " & _
                            "inner join CPE_ST_RewardOptions RO on RO.IncentiveID = I.IncentiveID and RO.TouchResponse=0 and RO.Deleted=0 " & _
                            "inner join CPE_ST_Deliverables D on D.RewardOptionID = RO.RewardOptionID and D.Deleted = 0 and DeliverableTypeID = 5 and RewardOptionPhase=3 " & _
                            "inner join CPE_ST_IncentiveCustomerGroups ICG on ICG.RewardOptionID=RO.RewardOptionID and ICG.Deleted = 0 " & _
        ...
        If (rst2.Rows.Count > 0) Then
          For Each row2 In rst2.Rows
            ...
            If (Not ExcludedFromOffer And ((Not OptOutOffer) Or (OptOutOffer And AllowOptOut))) Then
              ...
              Send("      <tr><td><form action=""home.aspx"" method=""post"">")
              Send("          <input type=""hidden"" name=""identifier"" id=""identifier"" value=""" & Identifier & """ />")
  ...
  Private Function RetrieveGraphicPath(ByVal OfferID As Long) As String
    ...
    Try
      ...
      MyCommon.QueryStr = "select OnScreenAdID, Name, ImageType, Width, Height from OnScreenAds where Deleted = 0 and OnScreenAdID in " & _
                          "(select OutputID from CPE_ST_Deliverables where Deleted = 0 and DeliverableTypeID = 1 and RewardOptionPhase = 1 " & _
                          "and RewardOptionID in (select RewardOptionID from CPE_ST_RewardOptions where IncentiveID = " & OfferID & " and TouchResponse = 0 and Deleted = 0))"
      rst = MyCommon.LRT_Select
      If (rst.Rows.Count > 0) Then
  ...
  Private Function RetrievePrintedMessage(ByVal OfferID As Long) As String
    ...
    MyCommon.Open_LogixRT()
    MyCommon.QueryStr = "select PMTypes.Description, PMTiers.TierLevel, " & _
                        "case when not(isnull(PMT.BodyText, '')='') then isnull(PMT.BodyText, '') else PMTiers.BodyText end as BodyText " & _
                        "from CPE_ST_PrintedMessages PM inner join CPE_ST_PrintedMessageTiers PMTiers on PM.MessageID = PMTiers.MessageID " & _
                        "inner join PrintedMessageTypes PMTypes on PM.MessageTypeID = PMTypes.TypeID " & _
                        "inner join CPE_ST_Deliverables D on D.OutputID = PM.MessageID " & _
                        "inner join CPE_ST_RewardOptions RO on RO.RewardOptionID = D.RewardOptionID " & _
                        "left join CPE_ST_PMTranslations as PMT with (NoLock) on PMT.PMTiersID=PMTiers.PKID and PMT.LanguageID=" & CustLanguageID & " " & _
  ...
  Public Sub Send_Legal(ByVal Handheld As Boolean)
    ...
    Send("     &copy; Copyright 2011, NCR Store, Inc.")
    Send("  </span>")
    Send("  <span id=""pptou"">")
  ...
  Private Function GenerateMessagePreview(ByVal newText As String, ByVal OfferID As Integer) As String
    ...
    If (OfferID > 0 AndAlso (newText.IndexOf("|OFFERSTART|") > -1 OrElse newText.IndexOf("|OFFEREND|") > -1)) Then
      ...
      MyCommon.QueryStr = "select StartDate, EndDate from CPE_ST_Incentives I with (NoLock) where I.IncentiveID=" & OfferID & " " & _
                          "union " & _
                          "select ProdStartDate as StartDate, ProdEndDate as EndDate from CM_ST_Offers O with (NoLock) " & _
    ...
    ' get the tags
    MyCommon.QueryStr = "select PrinterTypeID, '|' + tag + '|' as tag, isnull(pt.previewchars,'') as previewchars from MarkupTags as MT " & _
                        "left join PrinterTranslation as PT on MT.MarkupID=PT.MarkupID;"
    rst = MyCommon.LRT_Select
    ...
    For Each PrintLine In strArray
      ...
      For Each row In rst.Rows
        ...
        If (InStr(RawTextLine, PrinterTag) And MyCommon.NZ(row.Item("PrinterTypeID"), 0) = 4) Then
  ...
  Public Function DetectHandheld(ByRef MobileDevice As Boolean, ByRef Platform As String, ByRef UserAgent As String) As Boolean
    ...
    If MobileDevice = True Then
      Handheld = True
    ElseIf Platform.IndexOf("WinCE") > -1 OrElse Platform.IndexOf("Palm") > -1 OrElse Platform.IndexOf("Pocket") > -1 Then
      Handheld = True
  ...
  Public Sub Send_OffersForGroup(ByVal Handheld As Boolean)
    ...
    If (CustomerGroupID > 0) Then
      ...
      MyCommon.QueryStr = "select distinct O.OfferID, O.ExtOfferID, O.IsTemplate, O.CMOADeployStatus, O.StatusFlag, O.OddsOfWinning, O.InstantWin, " & _
                          "O.Name, O.Description, O.ProdStartDate, O.ProdEndDate, LinkID, OID.EngineID from CM_ST_Offers as O with (NoLock) " & _
                          "left join CM_ST_OfferConditions as OC on OC.OfferID=O.OfferID " & _
                          ...
                          "I.IncentiveName, Convert(nvarchar(2000),I.Description) as Description, I.StartDate, I.EndDate, ICG.CustomerGroupID, OID.EngineID from CPE_ST_Incentives as I with (NoLock) " & _
                          "left join CPE_ST_RewardOptions RO on I.IncentiveID=RO.IncentiveID and RO.TouchResponse=0 and RO.Deleted=0 " & _
                          "left join CPE_ST_IncentiveCustomerGroups ICG on RO.RewardOptionID=ICG.RewardOptionID and ICG.ExcludedUsers=0 " & _
